# Source Update Notes v1.62.0

v1.62.0 is the Event Structured Schema Fixture report-only checkpoint. It adds Event Structured Schema Fixture core/views/research-status/checkpoint documentation context and preserves Quant Research Design Pack v0.1 / Algorithm Timing Guard.

## Changed Or New Repo Files

- `src/quant_replay_system/event_structured_schema_fixture.py`
- `src/quant_replay_system/event_structured_schema_fixture_index.py`
- `src/quant_replay_system/event_structured_schema_fixture_health.py`
- `src/quant_replay_system/event_structured_schema_fixture_status.py`
- `src/quant_replay_system/local_research_dashboard.py`
- `src/quant_replay_system/cli.py`
- `tests/test_event_structured_schema_fixture.py`
- `tests/test_event_structured_schema_fixture_views.py`
- `tests/test_local_research_dashboard.py`
- `README.md`
- `docs/local_research_dashboard.md`
- `docs/event_structured_schema_fixture.md`
- `docs/release_checkpoint_v1.62.0.md`
- `docs/quant_research_design_pack_v0_1.md`
- `SOURCE_UPDATE_NOTES_v1_62_0.md`

## Semantics

`EVENT_STRUCTURED_SCHEMA_FIXTURE_CREATED` means synthetic/report-only event structured fixture artifacts exist for schema governance only.

Event structured rows are stable, versioned, PIT-governed records of public event candidates. They are not production event ingestion, not active event library, not real raw document ingestion, not real source adapter output, not factor observation, not production company exposure mapping, not replay evidence bundle, not signal_score implementation, not model training, not active weights, not active thresholds, not stock_profile validation, not paper validation, not real buy-review eligibility, not strategy performance validation, and not trading permission.

`event_time`, `publish_time`, and `available_time` remain distinct; `available_time` controls future replay eligibility. `confidence` is extraction/evidence confidence, not return probability. `direction` is contextual and may require company exposure.

This checkpoint does not create production event ingestion, active event library, real raw document ingestion, real source adapters/crawlers/connectors, LLM extraction runtime, factor observations, production company exposure mappings, replay evidence bundle, signal_score implementation, model training, active weights, active thresholds, stock_profile validation, paper validation, real buy-review, broker integration, API integration, order placement, message sending, or trading.
...
v1.62.0 preserves `docs/quant_research_design_pack_v0_1.md` and the Algorithm Timing Guard:
...
After the user manually commits and tags v1.62.0, ChatGPT should generate the external Project Source update package. Do not create that package in Git during this checkpoint.

ChatGPT should generate the external Project Source update after user tags v1.62.0.
