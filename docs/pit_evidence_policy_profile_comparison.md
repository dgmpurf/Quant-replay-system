...
`pit-evidence-policy-profile-comparison` compares the strict PIT evidence checklist result with an explicit opt-in `EOD_POST_CLOSE_LOW_BUDGET_PIT` policy profile.
...
`research-status` includes the latest policy-profile comparison as evidence-policy context. The unified summary exposes comparison id, status/stage, health status, profile name, row count, strict pass count, EOD low-budget pass count, relaxed blocker count, remaining blocked count, report path, and next action.
...
- Not-delisted, ST/no-ST, survivorship, reviewer, and evidence-reference blockers remain strict.
