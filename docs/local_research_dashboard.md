...
`research-status` includes Historical Replay Official Status Evidence Packet Closure Worklist context when status artifacts exist under:

```text
outputs/reports/manual_diagnostics/historical_replay_official_status_evidence_packet_closure_worklist_v0_1/status/
```

It exposes the latest worklist run id, selected signal date, universe, runtime status, health status, workflow stage, report path, row count, stock/ETF row counts, blocked count, missing official evidence count, manual-review count, no-hit review count, accepted no-hit context count, packet-row-ready-not-PIT-approved count, profile-conflict count, survivorship-warning count, listed/delisted/ST/suspension/universe/source/permission/revision/available-time missing counts, downstream safety flags, and the recommended next task.

Historical Replay Official Status Evidence Packet Closure Worklist context is context only. A packet row is not PIT approval. `packet_row_ready_not_pit_approved` is not PIT admissible. `no_hit_accepted_context` is not source reliability scoring. `source_hash_preview` is not source_hash validation, and `local_file_hash_preview` is not PIT evidence by itself. Same-day quotation presence is not automatically listed/not-delisted/no-ST/not-suspended/universe-membership proof. ETF ST not-applicable policy is required for ETF rows if no ST evidence applies. STOCK rows under legacy `etf_core` remain profile-conflict review context until separately resolved. Universe membership cannot be inferred from the legacy `etf_core` label alone. Forward returns remain future information.

This context is lower priority than later paper workflow context and must preserve `PAPER_WORKFLOW_READY` as the final workflow stage when paper workflow evidence exists. It does not close official evidence, close PIT evidence, approve PIT admissibility, create active replay input, run replay, freeze decisions, create labels, compute metrics, train models, create stock_profile or paper expansion, enable buy-review, authorize trading, run current-candidates, build snapshots, mutate signal semantics, call broker/order/message/API systems, or write `data/raw`, `data/processed`, or `data/cache`.
...
Header-only means structural metadata context only. It does not mean row count was computed, CSV data values were read, full CSV content was read, file byte hash was computed, `real_csv_consumed` became true, a package candidate was created, PIT admissibility was validated, active replay input was created, buy-review was allowed, or trading was authorized.
...
`research-status` includes `active-replay-input-ready-decision-status` as report-only ready-decision governance context when those artifacts exist.

Use `active-replay-input-ready-decision`, `active-replay-input-ready-decision-index`, `active-replay-input-ready-decision-health`, and `active-replay-input-ready-decision-status` to create, discover, safety-check, and summarize this report-only ready-decision context.

The unified summary records the latest ready-decision run id, status, health status, workflow stage, artifact path, ready-for-active-replay-input-ready-decision flag, report path, and next action. It also exports safety flags proving `active_replay_input_ready=false`, `active_replay_input=false`, `active_ready_emitted=false`, `replay_execution_allowed=false`, `forward_labels_allowed=false`, `training_allowed=false`, `stock_profile_allowed=false`, `buy_review_allowed=false`, `trading_allowed=false`, `replay_decisions_exist=false`, `forward_labels_exist=false`, `weights_trained=false`, `active_stock_profile_exists=false`, `real_buy_review_eligible=false`, `approval_applied=false`, no live trading, no broker API, no order placement, no messages, no LLM/API calls, no external API calls, no cache mutation, no `data/raw`, no `data/processed`, no `data/cache`, no current-candidates generation, no snapshot build, and no signal semantics change.

When the status reports `READY_FOR_ACTIVE_REPLAY_INPUT_READY_DECISION`, the dashboard treats it as ready-decision review context only. It is not active replay input and not `ACTIVE_REPLAY_INPUT_READY`. It must not be interpreted as replay permission, active-ready emission, paper approval, buy-review eligibility, performance validation, or trading authorization.
...
## Actual ACTIVE_REPLAY_INPUT_READY Marker-Only Emission Status

`research-status` includes `active-replay-input-ready-actual-emission-status` as report-only actual marker-only `ACTIVE_REPLAY_INPUT_READY` context when those artifacts exist.

Use `active-replay-input-ready-actual-emission`, `active-replay-input-ready-actual-emission-index`, `active-replay-input-ready-actual-emission-health`, and `active-replay-input-ready-actual-emission-status` to create, discover, safety-check, and summarize this marker-only diagnostics context.

The unified summary records the latest actual marker-only emission run id, status, health status, workflow stage, artifact path, marker-emitted flag, marker-file flag, marker-only-semantics flag, report path, and next action. It also exports safety flags proving `active_replay_input=false`, `replay_execution_allowed=false`, `replay_decisions_exist=false`, `forward_labels_allowed=false`, `forward_labels_exist=false`, `training_allowed=false`, `weights_trained=false`, `stock_profile_allowed=false`, `active_stock_profile_exists=false`, `buy_review_allowed=false`, `real_buy_review_eligible=false`, `trading_allowed=false`, `order_placed=false`, `broker_api_called=false`, no live trading, no broker API, no order placement, no messages, no LLM/API calls, no external API calls, no cache mutation, no `data/raw`, no `data/processed`, no `data/cache`, no current-candidates generation, no snapshot build, and no signal semantics change.
...
`research-status` includes `actual-replay-execute-status` as report-only actual replay execution context when those artifacts exist.

Use `actual-replay-execute`, `actual-replay-execute-index`, `actual-replay-execute-health`, and `actual-replay-execute-status` to create, discover, safety-check, and summarize actual replay execution artifacts.

The unified summary records the latest actual replay execution run id, status, health status, workflow stage, artifact path, source active input creation id, source real replay precheck id, execution booleans, report path, and next action. It also exports safety flags proving `replay_decisions_created=false`, `replay_decisions_exist=false`, `forward_labels_allowed=false`, `forward_labels_exist=false`, `training_allowed=false`, `weights_trained=false`, `stock_profile_allowed=false`, `active_stock_profile_exists=false`, `buy_review_allowed=false`, `real_buy_review_eligible=false`, `trading_allowed=false`, no live trading, no broker API, no order placement, no messages, no LLM/API calls, no external API calls, no cache mutation, no `data/raw`, no `data/processed`, no `data/cache`, no current-candidates generation, no snapshot build, and no signal semantics change.

When the status reports `ACTUAL_REPLAY_EXECUTED`, the dashboard treats it as execution artifacts only. It is not replay_decision creation. It does not compute forward labels. It does not train weights. It does not create active stock profiles. It does not create real buy-review eligibility. It does not authorize trading.
...
Use `active-replay-input-ready`, `active-replay-input-ready-index`, `active-replay-input-ready-health`, and `active-replay-input-ready-status` to create, discover, safety-check, and summarize this report-only active-ready workflow context.

The unified summary records the latest active-ready run id, status, health status, workflow stage, artifact path, ready-to-emit flag, report path, and next action. It also exports safety flags proving `active_replay_input_ready=false`, `active_replay_input=false`, `active_ready_emitted=false`, `replay_execution_allowed=false`, `replay_decisions_exist=false`, `forward_labels_allowed=false`, `forward_labels_exist=false`, `training_allowed=false`, `weights_trained=false`, `stock_profile_allowed=false`, `active_stock_profile_exists=false`, `buy_review_allowed=false`, `real_buy_review_eligible=false`, `trading_allowed=false`, `approval_applied=false`, `order_placed=false`, `broker_api_called=false`, no live trading, no broker API, no order placement, no messages, no LLM/API calls, no external API calls, no cache mutation, no `data/raw`, no `data/processed`, no `data/cache`, no current-candidates generation, no snapshot build, and no signal semantics change.

When the status reports `READY_TO_EMIT_ACTIVE_REPLAY_INPUT_READY`, the dashboard treats it as report-only active-ready governance context only. It is not active replay input and not `ACTIVE_REPLAY_INPUT_READY`. It must not be interpreted as replay permission, active-ready emission, paper approval, replay decisions, buy-review eligibility, performance validation, or trading authorization.

Active-ready workflow context is lower priority than later paper workflow and advisory artifacts. If later paper workflow artifacts exist, the final `workflow_stage` remains `PAPER_WORKFLOW_READY`; active-ready workflow fields remain visible for audit. The active-ready workflow does not emit `ACTIVE_REPLAY_INPUT_READY`, does not create active replay input, does not run replay, does not create replay decisions, does not compute forward labels, does not train weights, does not create active stock profiles, does not create real buy-review eligibility, does not authorize trading, and does not validate strategy performance.
...
`research-status` includes `active-model-status` as report-only Active Model Phase 1 research-governed context when those artifacts exist.

Use `active-model`, `active-model-index`, `active-model-health`, and `active-model-status` to create, discover, safety-check, and summarize report-only active-model reference artifacts.

The unified summary records the latest active-model run id, status/stage, health status, source model-weight-versioning lineage, model weight reference id, model version id, parameter version id, artifact flags, report path, next action, and downstream safety fields.

`ACTIVE_MODEL_RESEARCH_GOVERNED_ARTIFACTS_CREATED` is report-only research-governed artifact creation. It is not active production serving, not promoted model, not production model, not active thresholds, not advisory predictions, not active probabilities, not stock_profile, not buy-review, not paper approval, not performance validation, not current-candidates, not snapshot, not signal_semantics, and not trading.
...
`research-status` includes `paper-workflow-phase1-status` as report-only Paper Workflow Phase 1 research-governed context when those artifacts exist.

Use `paper-workflow-phase1`, `paper-workflow-phase1-index`, `paper-workflow-phase1-health`, and `paper-workflow-phase1-status` to create, discover, safety-check, and summarize report-only paper workflow phase-1 artifacts.

The unified summary records the latest Paper Workflow Phase 1 run id, status/stage, health status, source stock-profile lineage, source active-model lineage, source model-weight-versioning lineage, model reference ids, paper workflow metadata/input/lineage/review-context/draft/queue/limitations/warning/safety artifact flags, report path, next action, and downstream safety fields.

`PAPER_WORKFLOW_PHASE1_REPORT_ONLY_ARTIFACTS_CREATED` is report-only research-governed artifact creation. It is not APPROVED_FOR_PAPER, not real buy-review, not strategy performance validation, not current-candidates, not snapshot, not signal_semantics, not active stock_profile, not promoted model, not production model, not active thresholds, not advisory predictions, not active probabilities, and not trading.

Later paper workflow artifacts keep `PAPER_WORKFLOW_READY`; Paper Workflow Phase 1 fields remain visible as context. Health failures remain actionable when this layer is active, especially if an artifact claims `APPROVED_FOR_PAPER`, real buy-review eligibility, strategy performance validation, current-candidates generation, snapshot builds, signal_semantics changes, active stock_profile creation, promoted model state, production model state, active thresholds, advisory predictions, active probabilities, broker/order/message/API/cache side effects, data writes, or trading authorization.
...
## Tiny PIT Real Reviewed Package Candidate Contract Fixture Context

`research-status` includes Tiny PIT Real Reviewed Package Candidate Contract Fixture synthetic context when `tiny-pit-real-reviewed-package-candidate-contract-fixture-status` artifacts exist. It exposes the latest fixture id, status/stage, health, artifact path, report path, case count, pass-candidate/warn/fail counts, blocker/warning counts, report-only flags, diagnostic-only flags, synthetic-only flags, recommended next task, and safety fields.

`REAL_REVIEWED_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY` is a context stage only. It is not a real reviewed CSV package, not an active reviewed input candidate, not real replay input, not active replay input, not `ACTIVE_REPLAY_INPUT_READY`, not replay execution, not replay evidence bundles, not replay decisions, not replay decision freezes, not forward labels, not future-label joins, not training datasets, not metric computation, not signal_score inputs, not model training, not active weights, not active thresholds, not stock_profile validation, not paper validation, not real buy-review eligibility, not buy_review_allowed, not strategy performance validation, and not trading.
