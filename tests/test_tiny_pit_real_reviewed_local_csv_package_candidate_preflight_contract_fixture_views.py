...
def _build_run(tmp_path: Path):
    ...
    return write_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_artifacts(result)
...
def test_index_discovers_run_ignores_view_dirs_and_reports_malformed_metadata(tmp_path: Path) -> None:
    ...
    result = build_tiny_pit_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_index(
        root=root,
    ...
    assert result.latest_status == REAL_REVIEWED_LOCAL_CSV_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY
    assert result.latest_health_status == "PASS"
    assert (
        result.latest_workflow_stage
        == TINY_PIT_REAL_REVIEWED_LOCAL_CSV_PACKAGE_CANDIDATE_PREFLIGHT_CONTRACT_FIXTURE_CREATED_REPORT_ONLY
    )
    assert result.latest_case_count >= 55
    ...
    assert rows[0]["real_csv_required"] == "False"
    assert rows[0]["real_csv_consumed"] == "False"
    assert rows[0]["real_package_candidate_created"] == "False"
...
def test_health_passes_for_valid_synthetic_fixture_run(tmp_path: Path) -> None:
    ...
    result = check_tiny_pit_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_health(
        root=root,
...
def test_status_selects_latest_artifact_deterministically_and_preserves_safe_boundaries(tmp_path: Path) -> None:
    ...
    assert result.latest_tiny_pit_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_id == "zzz_second"
    assert (
        result.latest_tiny_pit_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_status
        == REAL_REVIEWED_LOCAL_CSV_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY
    )
    assert result.latest_tiny_pit_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_health_status == "PASS"
...
def test_cli_core_index_health_status_smoke_remains_report_only(tmp_path: Path) -> None:
    ...
    assert f"status: {REAL_REVIEWED_LOCAL_CSV_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY}" in core.stdout
    assert "health_status: PASS" in core.stdout
    assert "real_csv_required: False" in core.stdout
    ...
    assert f"latest_status: {REAL_REVIEWED_LOCAL_CSV_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY}" in index.stdout
    ...
    assert (
        ...
        f"{REAL_REVIEWED_LOCAL_CSV_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY}"
    ) in status.stdout
    assert (
        "latest_tiny_pit_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_active_replay_input: False"
        in status.stdout
    )
    assert (
        "latest_tiny_pit_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_trading_allowed: False"
        in status.stdout
    )
    assert (
...
def test_no_artifact_writes_to_data_or_docs_project_sources(tmp_path: Path) -> None:
    ...
    artifact_dir = next(path for path in root.iterdir() if path.is_dir() and path.name not in {"index", "health", "status"})
    assert set(ARTIFACT_FILENAMES.values()).issubset({path.name for path in artifact_dir.iterdir()})
    assert not (tmp_path / "data" / "raw").exists()
    assert not (tmp_path / "data" / "processed").exists()
    assert not (tmp_path / "data" / "cache").exists()
    assert not (tmp_path / "docs" / "project_sources").exists()
