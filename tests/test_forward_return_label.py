...
def test_only_core_cli_is_added_without_views_research_status_checkpoint_or_project_source() -> None:
    ...
    assert "forward-return-label-index" not in command_names
    assert "forward-return-label-health" not in command_names
    assert "forward-return-label-status" not in command_names
