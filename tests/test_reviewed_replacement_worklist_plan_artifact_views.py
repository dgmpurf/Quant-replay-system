...
def test_index_detects_replacement_plan_artifacts(tmp_path: Path) -> None:
    root = _build_replacement_plan_root(tmp_path)

    result = build_reviewed_replacement_worklist_plan_index(root=root, output_dir=tmp_path / "index")
    ...
    assert row["mixed_demo_core_row_count"] == 0
    assert row["active_worklist_mutated"] == False  # noqa: E712
    assert row["no_approval_applied"] == True  # noqa: E712


def test_health_passes_for_safe_replacement_plan(tmp_path: Path) -> None:
    root = _build_replacement_plan_root(tmp_path)

    result = check_reviewed_replacement_worklist_plan_health(root=root, output_dir=tmp_path / "health")
...
def test_health_fails_if_active_worklist_mutated_or_approval_detected(tmp_path: Path) -> None:
    root = _build_replacement_plan_root(tmp_path)
...
def test_status_summarizes_latest_replacement_plan(tmp_path: Path) -> None:
    root = _build_replacement_plan_root(tmp_path)

    result = run_reviewed_replacement_worklist_plan_status(root=root, output_dir=tmp_path / "status")

    assert result.status == "PASS"
    assert result.workflow_stage == "REVIEWED_REPLACEMENT_WORKLIST_PLAN_READY"
    assert result.row_count == 3
    assert result.stock_core_row_count == 2
    assert result.etf_core_row_count == 1


def test_research_status_includes_replacement_plan_and_preserves_paper_priority(tmp_path: Path) -> None:
    root = tmp_path / "reports"
    plan_root = _build_replacement_plan_root(tmp_path)
    _write_paper_workflow_status(root)

    result = run_local_research_dashboard(
        root=root,
        reviewed_replacement_worklist_plan_root=plan_root,
        output_dir=tmp_path / "dashboard",
    )

    assert result.workflow_stage == "PAPER_WORKFLOW_READY"
    assert result.latest_reviewed_replacement_worklist_plan_id
    assert result.reviewed_replacement_worklist_plan_status == "PASS"
    assert result.reviewed_replacement_worklist_plan_stage == "REVIEWED_REPLACEMENT_WORKLIST_PLAN_READY"
    assert result.reviewed_replacement_worklist_plan_stock_core_row_count == 2
    assert result.reviewed_replacement_worklist_plan_etf_core_row_count == 1
    exported = pd.read_csv(result.artifact_paths["local_research_summary"], dtype=str).fillna("")
    metadata = json.loads(result.artifact_paths["metadata"].read_text(encoding="utf-8"))
    assert exported.iloc[0]["reviewed_replacement_worklist_plan_stock_core_row_count"] == "2"
    assert metadata["reviewed_replacement_worklist_plan_stock_core_row_count"] == 2


def test_cli_index_health_status_and_research_status_work(tmp_path: Path, capsys) -> None:
    root = _build_replacement_plan_root(tmp_path)

    assert cli.main(["reviewed-replacement-worklist-plan-index", "--root", str(root), "--output-dir", str(tmp_path / "index")]) == 0
    assert "artifact_count: 1" in capsys.readouterr().out

    assert cli.main(["reviewed-replacement-worklist-plan-health", "--root", str(root), "--output-dir", str(tmp_path / "health")]) == 0
    assert "Health status: PASS" in capsys.readouterr().out

    assert cli.main(["reviewed-replacement-worklist-plan-status", "--root", str(root), "--output-dir", str(tmp_path / "status")]) == 0
    assert "workflow_stage: REVIEWED_REPLACEMENT_WORKLIST_PLAN_READY" in capsys.readouterr().out
    ...
    assert "latest_reviewed_replacement_worklist_plan_id:" in capsys.readouterr().out
...
def _build_replacement_plan_root(tmp_path: Path) -> Path:
    split_plan = _build_split_plan(tmp_path)
    root = tmp_path / "replacement_plans"
