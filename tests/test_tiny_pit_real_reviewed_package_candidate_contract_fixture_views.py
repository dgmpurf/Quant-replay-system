...
def _build_run(tmp_path: Path):
    return build_real_reviewed_package_candidate_contract_fixture_artifacts(output_root=_root(tmp_path))
...
def test_index_discovers_run_ignores_view_dirs_and_reports_malformed_metadata(tmp_path: Path) -> None:
    ...
    result = build_tiny_pit_real_reviewed_package_candidate_contract_fixture_index(root=root, output_dir=root / "index")
    rows = _read_csv(result.artifact_paths["index_csv"])

    assert result.artifact_count == 1
    assert result.latest_fixture_id == run.fixture_id
    assert result.latest_status == REAL_REVIEWED_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY
    assert result.latest_health_status == "PASS"
    assert result.latest_workflow_stage == TINY_PIT_REAL_REVIEWED_PACKAGE_CANDIDATE_CONTRACT_FIXTURE_CREATED_REPORT_ONLY
    assert result.latest_case_count >= 60
    ...
    assert rows[0]["real_replay_input_created"] == "False"
    assert rows[0]["active_replay_input"] == "False"
    assert rows[0]["replay_execution_allowed"] == "False"
    assert rows[0]["buy_review_allowed"] == "False"
    assert rows[0]["trading_allowed"] == "False"
    assert rows[0]["data_raw_written"] == "False"
...
def test_health_passes_for_valid_synthetic_fixture_run(tmp_path: Path) -> None:
    ...
    result = check_tiny_pit_real_reviewed_package_candidate_contract_fixture_health(root=root, output_dir=root / "health")
...
def test_health_fails_for_missing_required_artifact(tmp_path: Path) -> None:
    root = _root(tmp_path)
    run = _build_run(tmp_path)
    run.artifact_paths["field_family_contract"].unlink()
...
def test_health_fails_for_malformed_metadata(tmp_path: Path) -> None:
    root = _root(tmp_path)
    run = _build_run(tmp_path)
    run.artifact_paths["metadata"].write_text("{bad json", encoding="utf-8")
...
def test_health_fails_for_invalid_status_and_forbidden_status_wording(tmp_path: Path) -> None:
    root = _root(tmp_path)
    run = _build_run(tmp_path)
    metadata = _metadata(run.artifact_path)
    metadata["status"] = "ACTIVE_REPLAY_INPUT_READY"
    _write_metadata(run.artifact_path, metadata)
    run.artifact_paths["report"].write_text("This would grant READY_FOR_REPLAY permission.", encoding="utf-8")
    ...
    assert "FORBIDDEN_STATUS_WORDING" in codes
...
def test_status_selects_latest_artifact_deterministically_and_preserves_safe_boundaries(tmp_path: Path) -> None:
    ...
    result = run_tiny_pit_real_reviewed_package_candidate_contract_fixture_status(root=root, output_dir=root / "status")

    assert result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_id == "zzz_second"
    assert (
        result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_status
        == REAL_REVIEWED_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY
    )
    assert result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_health_status == "PASS"
    assert (
        result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_workflow_stage
        == TINY_PIT_REAL_REVIEWED_PACKAGE_CANDIDATE_CONTRACT_FIXTURE_CREATED_REPORT_ONLY
    )
    ...
    assert result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_real_reviewed_csv_package_created is False
    assert result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_active_reviewed_input_candidate_created is False
    assert result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_real_replay_input_created is False
...
def test_status_no_artifact_case_is_non_active(tmp_path: Path) -> None:
    ...
    assert result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_status == "NO_REAL_REVIEWED_PACKAGE_CANDIDATE"
    assert result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_active_replay_input is False
    assert result.latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_trading_allowed is False
...
def test_cli_core_index_health_status_smoke_remains_report_only(tmp_path: Path) -> None:
    ...
    assert f"status: {REAL_REVIEWED_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY}" in core.stdout
    ...
    assert "ACTIVE_REPLAY_INPUT_READY,true" not in core.stdout
    ...
    assert f"latest_status: {REAL_REVIEWED_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY}" in index.stdout
    ...
    assert (
        ...
        f"{REAL_REVIEWED_PACKAGE_CANDIDATE_SCHEMA_DESIGNED_REPORT_ONLY}"
    ) in status.stdout
    assert "latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_active_replay_input: False" in status.stdout
    assert "latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_trading_allowed: False" in status.stdout
    assert "latest_tiny_pit_real_reviewed_package_candidate_contract_fixture_buy_review_allowed: False" in status.stdout
    assert "ACTIVE_REPLAY_INPUT_READY,true" not in status.stdout
    assert "trading_allowed: True" not in status.stdout
    assert "buy_review_allowed: True" not in status.stdout
...
def test_no_artifact_writes_to_data_or_docs_project_sources(tmp_path: Path) -> None:
    root = _root(tmp_path)
    _build_run(tmp_path)
    build_tiny_pit_real_reviewed_package_candidate_contract_fixture_index(root=root, output_dir=root / "index")
    check_tiny_pit_real_reviewed_package_candidate_contract_fixture_health(root=root, output_dir=root / "health")
    run_tiny_pit_real_reviewed_package_candidate_contract_fixture_status(root=root, output_dir=root / "status")

    assert {path.name for path in root.iterdir() if path.is_dir()} >= {"index", "health", "status"}
    assert set(ARTIFACT_FILENAMES.values()).issubset(
        {path.name for path in next(path for path in root.iterdir() if path.is_dir() and path.name not in {"index", "health", "status"}).iterdir()}
    )
    assert not (tmp_path / "data" / "raw").exists()
    assert not (tmp_path / "data" / "processed").exists()
    assert not (tmp_path / "data" / "cache").exists()
