...
def test_index_detects_activation_artifacts(tmp_path: Path) -> None:
    root = _build_activation_root(tmp_path)

    result = build_reviewed_replacement_worklist_activation_index(root=root, output_dir=tmp_path / "index")
    ...
    assert row["activation_acknowledged"] == True  # noqa: E712
    assert row["active_worklist_mutated"] == False  # noqa: E712
    assert row["no_approval_applied"] == True  # noqa: E712


def test_health_passes_for_safe_activation(tmp_path: Path) -> None:
    root = _build_activation_root(tmp_path)

    result = check_reviewed_replacement_worklist_activation_health(root=root, output_dir=tmp_path / "health")
...
def test_status_summarizes_latest_activation(tmp_path: Path) -> None:
    root = _build_activation_root(tmp_path)

    result = run_reviewed_replacement_worklist_activation_status(root=root, output_dir=tmp_path / "status")

    assert result.status == "PASS"
    assert result.workflow_stage == "REVIEWED_REPLACEMENT_WORKLIST_ACTIVATED_AS_PLANNING_CONTEXT"
...
def test_research_status_includes_activation_and_preserves_paper_priority(tmp_path: Path) -> None:
    root = tmp_path / "reports"
    activation_root = _build_activation_root(tmp_path)
    _write_paper_workflow_status(root)

    result = run_local_research_dashboard(
        root=root,
        reviewed_replacement_worklist_activation_root=activation_root,
        output_dir=tmp_path / "dashboard",
    )

    assert result.workflow_stage == "PAPER_WORKFLOW_READY"
    assert result.latest_reviewed_replacement_worklist_activation_id
    assert result.reviewed_replacement_worklist_activation_status == "PASS"
    assert result.reviewed_replacement_worklist_activation_stage == (
        "REVIEWED_REPLACEMENT_WORKLIST_ACTIVATED_AS_PLANNING_CONTEXT"
    )
    assert result.reviewed_replacement_worklist_activation_stock_core_row_count == 2
    assert result.reviewed_replacement_worklist_activation_active_worklist_mutated is False
    exported = pd.read_csv(result.artifact_paths["local_research_summary"], dtype=str).fillna("")
    metadata = json.loads(result.artifact_paths["metadata"].read_text(encoding="utf-8"))
    assert exported.iloc[0]["reviewed_replacement_worklist_activation_stock_core_row_count"] == "2"
    assert metadata["reviewed_replacement_worklist_activation_stock_core_row_count"] == 2
...
def test_cli_index_health_status_and_research_status_work(tmp_path: Path, capsys) -> None:
    ...
    assert "workflow_stage: REVIEWED_REPLACEMENT_WORKLIST_ACTIVATED_AS_PLANNING_CONTEXT" in capsys.readouterr().out
    ...
    assert "latest_reviewed_replacement_worklist_activation_id:" in capsys.readouterr().out
...
def _build_activation_root(tmp_path: Path) -> Path:
    ...
    acceptance = build_reviewed_replacement_worklist_acceptance(
        ...
        output_dir=acceptance_root,
    )
    root = tmp_path / "activation"
    build_reviewed_replacement_worklist_activation(
        acceptance=acceptance.artifact_paths["reviewed_replacement_worklist_acceptance"],
