"""Status summary for report-only active replay input promotion artifacts."""
...
NO_PROMOTION_ARTIFACT_FOUND = "NO_PROMOTION_ARTIFACT_FOUND"
PROMOTION_NO_INPUT = "PROMOTION_NO_INPUT"
PROMOTION_BLOCKED = "PROMOTION_BLOCKED"
PROMOTION_HEALTH_FAILED = "PROMOTION_HEALTH_FAILED"

SUMMARY_COLUMNS = [
    "latest_promotion_run_id",
    "status",
    "health_status",
    "workflow_stage",
...
class ActiveReplayInputPromotionStatusResult:
    latest_promotion_run_id: str
    status: str
    health_status: str
    workflow_stage: str
...
def run_active_replay_input_promotion_status(
    *,
    root: str | Path = "outputs/reports/manual_diagnostics/active_replay_input_promotion_v0_1",
    output_dir: str | Path = "outputs/reports/manual_diagnostics/active_replay_input_promotion_v0_1/status",
) -> ActiveReplayInputPromotionStatusResult:
    sibling_root = Path(output_dir).parent
    index = build_active_replay_input_promotion_index(root=root, output_dir=sibling_root / "index")
    health = check_active_replay_input_promotion_health(root=root, output_dir=sibling_root / "health")
...
def _result_from_latest(
    ...
    summary = {
        "latest_promotion_run_id": _text(latest.get("promotion_run_id")),
        "status": status,
        "health_status": health_status,
        "workflow_stage": stage,
        "ready_for_human_review": _to_bool(latest.get("ready_for_human_review")),
        ...
        "report_path": _text(latest.get("promotion_report_path")),
...
def _no_artifact_result(
    ...
    summary = {
        ...
        "status": "MISSING",
        "health_status": health_status,
        "workflow_stage": NO_PROMOTION_ARTIFACT_FOUND,
        ...
        "report_path": "",
        "safety_statement": _safety_statement(),
        "next_action": "Run active-replay-input-promotion before artifact views.",
    }
    return _result(summary, output_dir, root, [f"No promotion artifacts found under {root}"])
...
def _result(
    ...
    paths = {
        ...
        "status_csv": Path(output_dir) / "active_replay_input_promotion_status.csv",
        "status_report": Path(output_dir) / "active_replay_input_promotion_status_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    frame = pd.DataFrame([summary], columns=SUMMARY_COLUMNS)
    return ActiveReplayInputPromotionStatusResult(
        latest_promotion_run_id=str(summary["latest_promotion_run_id"]),
        status=str(summary["status"]),
        health_status=str(summary["health_status"]),
        workflow_stage=str(summary["workflow_stage"]),
        ready_for_human_review=bool(summary["ready_for_human_review"]),
        active_replay_input_ready=False,
        active_replay_input=False,
        active_ready_emitted=False,
        forward_labels_exist=False,
...
def _write(result: ActiveReplayInputPromotionStatusResult) -> None:
    ...
    paths["status_report"].write_text(
        "\n".join(
            [
                ...
                f"- latest_promotion_run_id: {result.latest_promotion_run_id}",
    ...
    paths["metadata"].write_text(
        json.dumps(
            {
                "latest_promotion_run_id": result.latest_promotion_run_id,
...
def _stage_for_latest(status: str, health_status: str) -> str:
    if health_status == "FAIL":
        return PROMOTION_HEALTH_FAILED
    if status == NO_PROMOTION_INPUT:
        return PROMOTION_NO_INPUT
    if status == PROMOTION_READY_FOR_HUMAN_REVIEW:
        return PROMOTION_READY_FOR_HUMAN_REVIEW
    return PROMOTION_BLOCKED


def _next_action(stage: str) -> str:
    if stage == PROMOTION_READY_FOR_HUMAN_REVIEW:
        return "Review promotion artifact manually; do not treat it as active replay input."
    if stage == PROMOTION_NO_INPUT:
        return "Provide validator, smoke, promotion request, and human review manifests."
    if stage == PROMOTION_HEALTH_FAILED:
        return "Resolve promotion health blockers before using promotion context."
    if stage == NO_PROMOTION_ARTIFACT_FOUND:
        return "Run active-replay-input-promotion before artifact views."
    return "Resolve promotion blockers; do not run replay or downstream workflows."


def _safety_statement() -> str:
    return (
        "This promotion workflow is report-only. PROMOTION_READY_FOR_HUMAN_REVIEW is "
        "not ACTIVE_REPLAY_INPUT_READY. It does not create active replay input. It does "
