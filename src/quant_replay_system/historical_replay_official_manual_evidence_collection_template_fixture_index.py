"""Index view for official manual evidence collection template fixture artifacts."""
...
STATUS_INDEX_CREATED = "OFFICIAL_MANUAL_EVIDENCE_COLLECTION_TEMPLATE_FIXTURE_INDEX_CREATED_REPORT_ONLY"
NEXT_TASK = (
...
INDEX_COLUMNS = [
    ...
    "stock_row_count",
    "etf_row_count",
    "evidence_collection_template_row_count",
    "source_lineage_template_row_count",
    "no_hit_template_row_count",
    "survivorship_template_row_count",
    "reviewer_notes_template_row_count",
...
class HistoricalReplayOfficialManualEvidenceCollectionTemplateFixtureIndexResult:
    status: str
    artifact_count: int
...
def build_historical_replay_official_manual_evidence_collection_template_fixture_index(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path | None = None,
) -> HistoricalReplayOfficialManualEvidenceCollectionTemplateFixtureIndexResult:
    root_path = Path(root)
    ...
    result = HistoricalReplayOfficialManualEvidenceCollectionTemplateFixtureIndexResult(
...
def _row_from_artifact_dir(artifact_dir: Path) -> dict[str, Any] | None:
    ...
    row: dict[str, Any] = {
        ...
        "symbols_preview": _symbols_preview(artifact_dir / OUTPUT_FILES["evidence_collection_template"]),
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        ...
        "index_csv": output_dir / "historical_replay_official_manual_evidence_collection_template_fixture_index.csv",
        "index_md": output_dir / "historical_replay_official_manual_evidence_collection_template_fixture_index.md",
        "metadata_json": output_dir / "metadata.json",
    }


def _write(result: HistoricalReplayOfficialManualEvidenceCollectionTemplateFixtureIndexResult) -> None:
...
def _render_markdown(result: HistoricalReplayOfficialManualEvidenceCollectionTemplateFixtureIndexResult) -> str:
    lines = [
        "# Historical Replay Official Manual Evidence Collection Template Fixture Index",
        "",
        f"- Status: `{result.status}`",
        f"- Artifact count: `{result.artifact_count}`",
        "- Report-only index; no official evidence collection, acceptance, closure, PIT approval, replay, buy-review, or trading behavior is created.",
        "",
        "| run_id | status | health_status | evidence_template_rows | source_lineage_rows | symbols |",
        "| --- | --- | --- | --- | --- | --- |",
    ]
    for row in result.rows:
        lines.append(
            "| {run_id} | {runtime_status} | {health_status} | {evidence_collection_template_row_count} | {source_lineage_template_row_count} | {symbols_preview} |".format(
                **row
            )
        )
    return "\n".join(lines) + "\n"
