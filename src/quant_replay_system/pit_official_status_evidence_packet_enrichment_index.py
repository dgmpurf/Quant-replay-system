"""Index PIT official status evidence packet enrichment artifacts."""
...
def scan_pit_official_status_evidence_packet_enrichment_artifacts(
    ...
    for artifact_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        rows.append(
            {
                "artifact_type": "PIT_OFFICIAL_STATUS_EVIDENCE_PACKET_ENRICHMENT",
                "enrichment_id": _string(metadata.get("enrichment_id")) or artifact_dir.name,
                ...
                "strong_official_date_specific_quotation_count": _int(metadata.get("strong_official_date_specific_quotation_count")),
                "reviewed_no_hit_context_supported_count": _int(metadata.get("reviewed_no_hit_context_supported_count")),
                "reviewer_acceptance_required_count": _int(metadata.get("reviewer_acceptance_required_count")),
                "prior_official_symbol_level_context_count": _int(metadata.get("prior_official_symbol_level_context_count")),
                "local_eod_cache_context_count": _int(metadata.get("local_eod_cache_context_count")),
                ...
                "enrichment_only": _bool(metadata.get("enrichment_only")),
                "report_path": str(_output_path(artifact_dir, outputs.get("report"), "report.md")),
                "enriched_csv_path": str(_output_path(artifact_dir, outputs.get("enriched_csv"), "pit_official_status_evidence_packet_enrichment.csv")),
                "summary_csv_path": str(_output_path(artifact_dir, outputs.get("summary_csv"), "pit_official_status_evidence_packet_enrichment_summary.csv")),
                "blocker_matrix_path": str(_output_path(artifact_dir, outputs.get("blocker_matrix"), "remaining_enrichment_blockers.csv")),
...
def build_pit_official_status_evidence_packet_enrichment_index(
    *,
    root: str | Path = "outputs/reports/pit_official_status_evidence_packet_enrichment",
    output_dir: str | Path = "outputs/reports/pit_official_status_evidence_packet_enrichment/index",
) -> dict[str, Any]:
    frame = scan_pit_official_status_evidence_packet_enrichment_artifacts(root)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    index_csv = output_dir / "pit_official_status_evidence_packet_enrichment_index.csv"
    report = output_dir / "pit_official_status_evidence_packet_enrichment_index_report.md"
    ...
    report.write_text(f"# PIT Official Status Evidence Packet Enrichment Index\n\nartifact_count: {len(frame)}\n", encoding="utf-8")
...
def _finalize(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in INDEX_COLUMNS:
        if column not in frame.columns:
            frame[column] = ""
    return frame[INDEX_COLUMNS].sort_values(["created_at", "enrichment_id"], ascending=[False, False]).reset_index(drop=True)
