"""Index for material PIT evidence gate closure plan artifacts."""
...
class MaterialPitEvidenceGateClosurePlanIndexResult:
    artifact_count: int
...
def build_material_pit_evidence_gate_closure_plan_index(
    *,
    root: str | Path = "outputs/reports/material_pit_evidence_gate_closure_plan",
    output_dir: str | Path = "outputs/reports/material_pit_evidence_gate_closure_plan/index",
) -> MaterialPitEvidenceGateClosurePlanIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows))
    paths = {
        "artifact_dir": Path(output_dir),
        "index_csv": Path(output_dir) / "material_pit_evidence_gate_closure_plan_index.csv",
        "index_report": Path(output_dir) / "material_pit_evidence_gate_closure_plan_index_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    result = MaterialPitEvidenceGateClosurePlanIndexResult(
    ...
    write_material_pit_evidence_gate_closure_plan_index(result)
    return result


def write_material_pit_evidence_gate_closure_plan_index(
    result: MaterialPitEvidenceGateClosurePlanIndexResult,
) -> dict[str, Path]:
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Material PIT evidence gate closure plan root does not exist: {root}"]
...
def _row_from_metadata(artifact_dir: Path, metadata_path: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "artifact_type": "MATERIAL_PIT_EVIDENCE_GATE_CLOSURE_PLAN",
        ...
        "reusable_symbol_level_closure_count": _to_int(metadata.get("reusable_symbol_level_closure_count")),
        "date_specific_closure_required_count": _to_int(metadata.get("date_specific_closure_required_count")),
        "reviewer_no_hit_acceptance_required_count": _to_int(metadata.get("reviewer_no_hit_acceptance_required_count")),
        "survivorship_rationale_required_count": _to_int(metadata.get("survivorship_rationale_required_count")),
        "metadata_closure_required_count": _to_int(metadata.get("metadata_closure_required_count")),
        "stock_st_no_st_required_count": _to_int(metadata.get("stock_st_no_st_required_count")),
        ...
        "planning_only": _to_bool(metadata.get("planning_only")),
        "report_path": str(Path(output_files.get("report") or artifact_dir / "report.md")),
        "plan_csv_path": str(
            Path(output_files.get("plan_csv") or artifact_dir / "material_pit_evidence_gate_closure_plan.csv")
        ),
