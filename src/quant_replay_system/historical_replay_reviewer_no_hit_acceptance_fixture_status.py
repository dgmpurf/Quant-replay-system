"""Status view for reviewer no-hit acceptance fixture artifacts."""
...
STATUS_CREATED = "REVIEWER_NO_HIT_ACCEPTANCE_FIXTURE_STATUS_CREATED_REPORT_ONLY"
STATUS_NO_ARTIFACTS = "REVIEWER_NO_HIT_ACCEPTANCE_FIXTURE_STATUS_NO_ARTIFACTS"
...
class HistoricalReplayReviewerNoHitAcceptanceFixtureStatusResult:
    status: str
    latest_run_id: str
    latest_status: str
...
def run_historical_replay_reviewer_no_hit_acceptance_fixture_status(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path | None = None,
) -> HistoricalReplayReviewerNoHitAcceptanceFixtureStatusResult:
    root_path = Path(root)
    out_dir = Path(output_dir) if output_dir is not None else root_path / "status"
    sibling_root = out_dir.parent
    index = build_historical_replay_reviewer_no_hit_acceptance_fixture_index(
        root=root_path, output_dir=sibling_root / "index"
    )
    health = check_historical_replay_reviewer_no_hit_acceptance_fixture_health(
    ...
    result = HistoricalReplayReviewerNoHitAcceptanceFixtureStatusResult(
        ...
        warnings=[] if "FAIL" not in str(summary.get("latest_health_status", "")) else [f"Reviewer no-hit acceptance fixture health is {health.status}."],
    )
    _write(result)
    return result
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        ...
        "status_csv": output_dir / "historical_replay_reviewer_no_hit_acceptance_fixture_status.csv",
        "status_md": output_dir / "historical_replay_reviewer_no_hit_acceptance_fixture_status.md",
        "metadata_json": output_dir / "metadata.json",
    }


def _write(result: HistoricalReplayReviewerNoHitAcceptanceFixtureStatusResult) -> None:
...
def _render_markdown(result: HistoricalReplayReviewerNoHitAcceptanceFixtureStatusResult) -> str:
    return "\n".join(
        [
            ...
            "- Report-only status; no no-hit evidence acceptance, PIT approval, replay, buy-review, trading, or protected data-write behavior is authorized.",
            "",
        ]
    )
