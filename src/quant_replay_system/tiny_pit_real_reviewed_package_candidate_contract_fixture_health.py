"""Health view for Tiny PIT real reviewed package candidate contract fixture artifacts."""
...
class TinyPitRealReviewedPackageCandidateContractFixtureHealthResult:
    status: str
...
def check_tiny_pit_real_reviewed_package_candidate_contract_fixture_health(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path = f"{DEFAULT_ROOT}/health",
) -> TinyPitRealReviewedPackageCandidateContractFixtureHealthResult:
    root_path = Path(root)
    ...
    result = TinyPitRealReviewedPackageCandidateContractFixtureHealthResult(
        status=status,
        ...
        warnings=[] if root_path.exists() else [f"Tiny PIT real reviewed package candidate contract fixture root does not exist: {root}"],
    )
    _write(result)
    return result
...
def _metadata_issues(fixture_id: str, metadata: dict[str, Any], path: Path) -> list[dict[str, Any]]:
    ...
    if status not in real_reviewed_package_candidate_contract_fixture_statuses():
        issues.append(_issue(fixture_id, "ERROR", "INVALID_STATUS", f"Invalid status: {status}", path))
    if _text(metadata.get("workflow_stage")) != TINY_PIT_REAL_REVIEWED_PACKAGE_CANDIDATE_CONTRACT_FIXTURE_CREATED_REPORT_ONLY:
...
def _write(result: TinyPitRealReviewedPackageCandidateContractFixtureHealthResult) -> None:
...
def _paths(output_dir: str | Path) -> dict[str, Path]:
    ...
    return {
        ...
        "health_csv": artifact_dir / "tiny_pit_real_reviewed_package_candidate_contract_fixture_health.csv",
