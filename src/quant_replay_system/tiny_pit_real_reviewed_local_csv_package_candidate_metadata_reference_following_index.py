"""Index view for Tiny PIT metadata-reference-following artifacts."""
...
class MetadataReferenceFollowingIndexResult:
    artifact_count: int
    latest_run_id: str
...
def build_metadata_reference_following_index(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path = f"{DEFAULT_ROOT}/index",
) -> MetadataReferenceFollowingIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows, dtype=object))
    latest = _latest_row(frame)
    paths = _paths(output_dir)
    result = MetadataReferenceFollowingIndexResult(
        artifact_count=len(frame),
        latest_run_id=_text(latest.get("run_id")),
        latest_runtime_status=_text(latest.get("runtime_status")) or "NO_REAL_REVIEWED_LOCAL_CSV_PACKAGE_CANDIDATE_METADATA_REFERENCE_INPUT",
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Metadata-reference-following root does not exist: {root}"]
    ...
    for artifact_dir in _candidate_dirs(root):
        ...
        try:
            ...
            warnings.append(f"Could not read metadata-reference-following metadata {metadata_path}: {exc}")
            continue
...
def _row_from_metadata(artifact_dir: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "input_mode": _text(metadata.get("input_mode")),
        "inspection_level": _text(metadata.get("inspection_level")),
        "csv_read_level": _text(metadata.get("csv_read_level")),
        "real_manifest_read": _to_bool(metadata.get("real_manifest_read")),
        "references_declared": _to_bool(metadata.get("references_declared")),
        "references_followed": _to_bool(metadata.get("references_followed")),
        "metadata_files_followed_count": _to_int(metadata.get("metadata_files_followed_count")),
        "forbidden_data_references_count": _to_int(metadata.get("forbidden_data_references_count")),
        "path_guard_blocker_count": _to_int(metadata.get("path_guard_blocker_count")),
        "manifest_schema_blocker_count": _to_int(metadata.get("manifest_schema_blocker_count")),
        "metadata_schema_blocker_count": _to_int(metadata.get("metadata_schema_blocker_count")),
        "available_time_metadata_blocker_count": _to_int(metadata.get("available_time_metadata_blocker_count")),
        "source_hash_revision_metadata_blocker_count": _to_int(metadata.get("source_hash_revision_metadata_blocker_count")),
        "reviewer_quality_metadata_blocker_count": _to_int(metadata.get("reviewer_quality_metadata_blocker_count")),
        "limitation_warning_count": _to_int(metadata.get("limitation_warning_count")),
        ...
        "report_path": _text(metadata.get("report_path") or artifact_dir / "metadata_reference_following_report.md"),
        "recommended_next_task": _text(metadata.get("recommended_next_task")),
    }


def _write(result: MetadataReferenceFollowingIndexResult) -> None:
...
def _paths(output_dir: str | Path) -> dict[str, Path]:
    ...
    return {
        ...
        "index_csv": artifact_dir / "tiny_pit_real_reviewed_local_csv_package_candidate_metadata_reference_following_index.csv",
