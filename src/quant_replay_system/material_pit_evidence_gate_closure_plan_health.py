"""Health checks for material PIT evidence gate closure plan artifacts."""
...
from quant_replay_system.material_pit_evidence_gate_closure_plan import PLAN_COLUMNS
...
class MaterialPitEvidenceGateClosurePlanHealthResult:
    status: str
...
def check_material_pit_evidence_gate_closure_plan_health(
    *,
    root: str | Path = "outputs/reports/material_pit_evidence_gate_closure_plan",
    output_dir: str | Path = "outputs/reports/material_pit_evidence_gate_closure_plan/health",
) -> MaterialPitEvidenceGateClosurePlanHealthResult:
    ...
    paths = {
        ...
        "health_csv": artifact_dir / "material_pit_evidence_gate_closure_plan_health.csv",
        "health_report": artifact_dir / "material_pit_evidence_gate_closure_plan_health_report.md",
        "metadata": artifact_dir / "metadata.json",
    }
    result = MaterialPitEvidenceGateClosurePlanHealthResult(
        status=status,
...
def _issues_for_row(row: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if plan_csv_path.exists():
        ...
        if len(frame) != 16:
            issues.append(_issue(plan_id, "ERROR", "UNEXPECTED_ROW_COUNT", f"Expected 16 rows, found {len(frame)}.", plan_csv_path))
        text = plan_csv_path.read_text(encoding="utf-8")
        if "APPROVED_FOR_PIT_UNIVERSE" in text:
            issues.append(_issue(plan_id, "ERROR", "APPROVED_FOR_PIT_UNIVERSE_DETECTED", "Plan artifacts must not contain approval status text.", plan_csv_path))
        if "symbol" in frame and "000001" not in set(frame["symbol"].astype(str)):
            issues.append(_issue(plan_id, "ERROR", "LEADING_ZERO_SYMBOL_NOT_PRESERVED", "Leading-zero symbol 000001 was not preserved.", plan_csv_path))
        for column, code in {
            ...
            if column in frame and frame[column].map(_to_bool).any():
                issues.append(_issue(plan_id, "ERROR", code, f"Unsafe column {column} contains true.", plan_csv_path))
        for column, code in {
            ...
            "planning_only": "PLANNING_ONLY_FLAG_MISSING",
        }.items():
            if column in frame and not frame[column].map(_to_bool).all():
                issues.append(_issue(plan_id, "ERROR", code, f"Required safety column {column} is not true for every row.", plan_csv_path))
    if _to_int(row.get("row_count")) != 16:
        issues.append(_issue(plan_id, "ERROR", "UNEXPECTED_METADATA_ROW_COUNT", "Metadata row_count must be 16.", metadata_path))
    for field, code in {
        "clean_review_updates_created": "CLEAN_REVIEW_UPDATES_DETECTED",
        "approval_applied": "APPROVAL_APPLIED_DETECTED",
        "pit_review_run": "PIT_REVIEW_RUN_DETECTED",
    ...
    if metadata_path.exists():
        ...
        for filename in ["review_updates.csv", "clean_review_updates.csv"]:
            ...
            if path.exists():
                issues.append(_issue(plan_id, "ERROR", "CLEAN_REVIEW_UPDATES_FILE_DETECTED", f"{filename} must not be created by this workflow.", path))
    return issues


def _write(result: MaterialPitEvidenceGateClosurePlanHealthResult) -> None:
