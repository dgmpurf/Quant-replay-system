"""Index report-only active replay input creation artifacts."""
...
DEFAULT_ROOT = Path("outputs/reports/manual_diagnostics/active_replay_input_create_v0_1")
...
class ActiveReplayInputCreateIndexResult:
    artifact_count: int
...
def build_active_replay_input_create_index(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path = DEFAULT_OUTPUT_DIR,
) -> ActiveReplayInputCreateIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows, dtype=object))
    paths = {
        "artifact_dir": Path(output_dir),
        "index_csv": Path(output_dir) / "active_replay_input_create_index.csv",
        "index_report": Path(output_dir) / "active_replay_input_create_index_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    result = ActiveReplayInputCreateIndexResult(
    ...
    write_active_replay_input_create_index(result)
    return result


def write_active_replay_input_create_index(result: ActiveReplayInputCreateIndexResult) -> dict[str, Path]:
    ...
    paths["index_report"].write_text(
        "\n".join(
            [
                "# Active Replay Input Creation Index",
                "",
                "Report-only active replay input creation index. ACTIVE_REPLAY_INPUT_CREATED means only a governed input package exists for a future separate replay workflow; it is not replay, replay decisions, labels, training, stock_profile, buy-review eligibility, broker/order/message/API/cache/data side effects, or trading.",
                "",
                f"- artifact_count: {result.artifact_count}",
                "",
                result.index_frame.to_markdown(index=False)
                if not result.index_frame.empty
                else "No active replay input creation artifacts found.",
            ]
        ),
        encoding="utf-8",
    )
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Active replay input creation root does not exist: {root}"]
    rows: list[dict[str, Any]] = []
    warnings: list[str] = []
    for artifact_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        if artifact_dir.name in {"index", "health", "status"} or artifact_dir.name.startswith("_"):
            continue
        metadata_path = artifact_dir / "active_input_creation_metadata.json"
        if not metadata_path.exists():
            continue
        try:
            metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            warnings.append(f"Could not read active input creation metadata {metadata_path}: {exc}")
            continue
        if _text(metadata.get("active_input_creation_run_id")):
...
def _row_from_metadata(artifact_dir: Path, metadata_path: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    active_input_path = Path(
        _text(artifact_paths.get("active_replay_input")) or str(artifact_dir / "active_replay_input.json")
    )
    active_input = _read_json(active_input_path)
    return {
        "active_input_creation_run_id": _text(metadata.get("active_input_creation_run_id")),
        ...
        "active_replay_input_created": _to_bool(metadata.get("active_replay_input_created")),
        "active_replay_input": _to_bool(metadata.get("active_replay_input")),
        "active_replay_input_file_exists": active_input_path.exists(),
        "source_marker_run_id": _text(active_input.get("source_marker_run_id")),
        "marker_status": _text(active_input.get("marker_status")),
        "active_replay_input_ready_marker_emitted": _to_bool(
            active_input.get("active_replay_input_ready_marker_emitted")
        ),
        "marker_file_exists": _to_bool(active_input.get("marker_file_exists")),
        "marker_only_semantics_confirmed": _to_bool(active_input.get("marker_only_semantics_confirmed")),
        "replay_as_of_date": _text(active_input.get("replay_as_of_date")),
        "pit_universe_ref": _text(active_input.get("pit_universe_ref")),
        "source_registry_ref": _text(active_input.get("source_registry_ref")),
        "evidence_bundle_ref": _text(active_input.get("evidence_bundle_ref")),
        "source_hash_coverage": _text(active_input.get("source_hash_coverage")),
        "revision_id_coverage": _text(active_input.get("revision_id_coverage")),
        "available_time_policy": _text(active_input.get("available_time_policy")),
        "taxonomy_coverage": _text(active_input.get("taxonomy_coverage")),
        ...
        "report_path": _text(artifact_paths.get("report")) or str(artifact_dir / "active_input_creation_report.md"),
        "metadata_path": str(metadata_path),
        "active_replay_input_path": str(active_input_path),
    }
...
def _finalize(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    return frame[INDEX_COLUMNS].sort_values(["generated_at", "active_input_creation_run_id"]).reset_index(drop=True)
