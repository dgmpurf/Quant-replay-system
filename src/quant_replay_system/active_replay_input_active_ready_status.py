"""Status summary for report-only active replay input active-ready artifacts."""
...
NO_ACTIVE_READY_ARTIFACT_FOUND = "NO_ACTIVE_READY_ARTIFACT_FOUND"
ACTIVE_READY_NO_INPUT = "ACTIVE_READY_NO_INPUT"
ACTIVE_READY_BLOCKED = "ACTIVE_READY_BLOCKED"
ACTIVE_READY_HEALTH_FAILED = "ACTIVE_READY_HEALTH_FAILED"
...
class ActiveReplayInputActiveReadyStatusResult:
...
def run_active_replay_input_active_ready_status(
    *,
    root: str | Path = "outputs/reports/manual_diagnostics/active_replay_input_active_ready_v0_1",
    output_dir: str | Path = "outputs/reports/manual_diagnostics/active_replay_input_active_ready_v0_1/status",
) -> ActiveReplayInputActiveReadyStatusResult:
    sibling_root = Path(output_dir).parent
    index = build_active_replay_input_active_ready_index(root=root, output_dir=sibling_root / "index")
    health = check_active_replay_input_active_ready_health(root=root, output_dir=sibling_root / "health")
...
def _result_from_latest(
    ...
    summary = {
        ...
        "ready_for_final_review": _to_bool(latest.get("ready_for_final_review")),
...
def _no_artifact_result(
    ...
    summary = {
        ...
        "status": "MISSING",
        "health_status": health_status,
        "workflow_stage": NO_ACTIVE_READY_ARTIFACT_FOUND,
        ...
        "report_path": "",
        "safety_statement": _safety_statement(),
        "next_action": "Run active-replay-input-active-ready before artifact views.",
    }
...
def _result(
    ...
    paths = {
        ...
        "status_csv": Path(output_dir) / "active_replay_input_active_ready_status.csv",
        "status_report": Path(output_dir) / "active_replay_input_active_ready_status_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    frame = pd.DataFrame([summary], columns=SUMMARY_COLUMNS)
    return ActiveReplayInputActiveReadyStatusResult(
        ...
        ready_for_final_review=bool(summary["ready_for_final_review"]),
        active_replay_input_ready=False,
        active_replay_input=False,
        active_ready_emitted=False,
        forward_labels_exist=False,
...
def _write(result: ActiveReplayInputActiveReadyStatusResult) -> None:
...
def _stage_for_latest(status: str, health_status: str) -> str:
    if health_status == "FAIL":
        return ACTIVE_READY_HEALTH_FAILED
    if status == NO_ACTIVE_READY_INPUT:
        return ACTIVE_READY_NO_INPUT
    if status == ACTIVE_READY_READY_FOR_FINAL_REVIEW:
        return ACTIVE_READY_READY_FOR_FINAL_REVIEW
    return ACTIVE_READY_BLOCKED


def _next_action(stage: str) -> str:
    if stage == ACTIVE_READY_READY_FOR_FINAL_REVIEW:
        return "Review active-ready final-review artifact manually; do not treat it as active replay input."
    if stage == ACTIVE_READY_NO_INPUT:
        return "Provide acceptance and active-ready governance manifests."
    if stage == ACTIVE_READY_HEALTH_FAILED:
        return "Resolve active-ready health blockers before using active-ready context."
    if stage == NO_ACTIVE_READY_ARTIFACT_FOUND:
        return "Run active-replay-input-active-ready before artifact views."
    return "Resolve active-ready blockers; do not run replay or downstream workflows."


def _safety_statement() -> str:
    return (
        "This active-ready workflow is report-only. ACTIVE_READY_READY_FOR_FINAL_REVIEW "
...
__all__ = [
    "NO_ACTIVE_READY_ARTIFACT_FOUND",
