"""Index view for Tiny PIT real reviewed LOCAL_CSV preflight fixture artifacts."""
...
class TinyPitRealReviewedLocalCsvPackageCandidatePreflightContractFixtureIndexResult:
    artifact_count: int
...
def build_tiny_pit_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_index(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path = f"{DEFAULT_ROOT}/index",
) -> TinyPitRealReviewedLocalCsvPackageCandidatePreflightContractFixtureIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows, dtype=object))
    latest = _latest_row(frame)
    paths = _paths(output_dir)
    result = TinyPitRealReviewedLocalCsvPackageCandidatePreflightContractFixtureIndexResult(
        artifact_count=len(frame),
        latest_fixture_id=_text(latest.get("fixture_id")),
        latest_status=_text(latest.get("status")) or "NO_REAL_REVIEWED_LOCAL_CSV_PACKAGE_CANDIDATE",
        latest_health_status=_text(latest.get("health_status")) or "WARN",
        latest_workflow_stage=(
            _text(latest.get("workflow_stage"))
            or NO_TINY_PIT_REAL_REVIEWED_LOCAL_CSV_PACKAGE_CANDIDATE_PREFLIGHT_CONTRACT_FIXTURE
        ),
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Tiny PIT real reviewed LOCAL_CSV preflight fixture root does not exist: {root}"]
    ...
    for artifact_dir in _candidate_dirs(root):
        ...
        try:
            ...
            warnings.append(f"Could not read Tiny PIT LOCAL_CSV preflight metadata {metadata_path}: {exc}")
            continue
...
def _row_from_metadata(artifact_dir: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "real_csv_required": _to_bool(metadata.get("real_csv_required")),
        ...
        "active_reviewed_input_candidate_created": _to_bool(
            metadata.get("active_reviewed_input_candidate_created")
        ),
        ...
        or str(artifact_dir / "real_reviewed_local_csv_package_candidate_preflight_contract_fixture_report.md"),
    }


def _write(result: TinyPitRealReviewedLocalCsvPackageCandidatePreflightContractFixtureIndexResult) -> None:
...
def _paths(output_dir: str | Path) -> dict[str, Path]:
    ...
    return {
        ...
        "index_csv": artifact_dir
        / "tiny_pit_real_reviewed_local_csv_package_candidate_preflight_contract_fixture_index.csv",
