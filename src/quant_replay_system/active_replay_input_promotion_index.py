"""Index report-only active replay input promotion artifacts."""
...
INDEX_COLUMNS = [
    ...
    "smoke_artifact_path",
    "promotion_request_manifest_path",
    "human_review_manifest_path",
    "status",
    "workflow_stage",
    ...
    "passed_human_review_gate_count",
    "blocked_human_review_gate_count",
    "issue_count",
    "blocker_count",
    "warning_count",
...
class ActiveReplayInputPromotionIndexResult:
    artifact_count: int
...
def build_active_replay_input_promotion_index(
    *,
    root: str | Path = "outputs/reports/manual_diagnostics/active_replay_input_promotion_v0_1",
    output_dir: str | Path = "outputs/reports/manual_diagnostics/active_replay_input_promotion_v0_1/index",
) -> ActiveReplayInputPromotionIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows, dtype=object))
    paths = {
        "artifact_dir": Path(output_dir),
        "index_csv": Path(output_dir) / "active_replay_input_promotion_index.csv",
        "index_report": Path(output_dir) / "active_replay_input_promotion_index_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    result = ActiveReplayInputPromotionIndexResult(
    ...
    write_active_replay_input_promotion_index(result)
    return result


def write_active_replay_input_promotion_index(
    result: ActiveReplayInputPromotionIndexResult,
) -> dict[str, Path]:
    ...
    paths["index_report"].write_text(
        "\n".join(
            [
                ...
                "",
                "Report-only promotion index. No active replay input, replay, current-candidates, snapshots, forward labels, training, active stock profiles, research-status integration, data writes, API calls, messages, broker integration, orders, or cache mutation was invoked.",
                "",
                f"- artifact_count: {result.artifact_count}",
                "",
                result.index_frame.to_markdown(index=False)
                if not result.index_frame.empty
                else "No promotion artifacts found.",
            ]
        ),
        encoding="utf-8",
    )
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Promotion root does not exist: {root}"]
    rows: list[dict[str, Any]] = []
    warnings: list[str] = []
    for artifact_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        if artifact_dir.name in {"index", "health", "status"} or artifact_dir.name.startswith("_"):
            continue
        metadata_path = artifact_dir / "promotion_metadata.json"
        if not metadata_path.exists():
            continue
        try:
            metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            warnings.append(f"Could not read promotion metadata {metadata_path}: {exc}")
            continue
        if _text(metadata.get("promotion_run_id")):
...
def _row_from_metadata(artifact_dir: Path, metadata_path: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "promotion_run_id": _text(metadata.get("promotion_run_id")),
        ...
        "smoke_artifact_path": _text(metadata.get("smoke_artifact_path")),
        "promotion_request_manifest_path": _text(metadata.get("promotion_request_manifest_path")),
        "human_review_manifest_path": _text(metadata.get("human_review_manifest_path")),
        "status": _text(metadata.get("status")),
        "workflow_stage": _text(metadata.get("workflow_stage")),
        "ready_for_human_review": _to_bool(metadata.get("ready_for_human_review")),
        ...
        "human_review_gate_count": _to_int(metadata.get("human_review_gate_count")),
        "passed_human_review_gate_count": _to_int(metadata.get("passed_human_review_gate_count")),
        "blocked_human_review_gate_count": _to_int(metadata.get("blocked_human_review_gate_count")),
        ...
        "promotion_report_path": _text(artifact_paths.get("promotion_report")) or str(artifact_dir / "promotion_report.md"),
...
__all__ = [
    ...
    "write_active_replay_input_promotion_index",
]
