"""Index for first-batch partial completion impact artifacts."""
...
class FirstBatchPartialCompletionImpactIndexResult:
    artifact_count: int
...
def build_first_batch_partial_completion_impact_index(
    *,
    root: str | Path = "outputs/reports/first_batch_partial_completion_impact",
    output_dir: str | Path = "outputs/reports/first_batch_partial_completion_impact/index",
) -> FirstBatchPartialCompletionImpactIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows))
    paths = {
        "artifact_dir": Path(output_dir),
        "index_csv": Path(output_dir) / "first_batch_partial_completion_impact_index.csv",
        "index_report": Path(output_dir) / "first_batch_partial_completion_impact_index_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    result = FirstBatchPartialCompletionImpactIndexResult(
    ...
    write_first_batch_partial_completion_impact_index(result)
    return result


def write_first_batch_partial_completion_impact_index(
    result: FirstBatchPartialCompletionImpactIndexResult,
) -> dict[str, Path]:
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"First-batch partial completion impact root does not exist: {root}"]
    ...
    for artifact_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        try:
            ...
            continue
        if _text(metadata.get("impact_id")):
...
def _row_from_metadata(artifact_dir: Path, metadata_path: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "artifact_type": "FIRST_BATCH_PARTIAL_COMPLETION_IMPACT",
        "impact_id": _text(metadata.get("impact_id")) or artifact_dir.name,
        "status": _text(metadata.get("status")),
        "completion_plan_id": _text(metadata.get("completion_plan_id")),
        "partial_completion_path": _text(metadata.get("partial_completion_path")),
        "row_count": _to_int(metadata.get("row_count")),
        "completed_row_count": _to_int(metadata.get("completed_row_count")),
        "completed_field_count": _to_int(metadata.get("completed_field_count")),
        "blocker_reduced_count": _to_int(metadata.get("blocker_reduced_count")),
        "material_blocker_reduced_count": _to_int(metadata.get("material_blocker_reduced_count")),
        ...
        "impact_only": _to_bool(metadata.get("impact_only")),
        "report_path": str(Path(output_files.get("report") or artifact_dir / "report.md")),
        "impact_csv_path": str(Path(output_files.get("impact_csv") or artifact_dir / "first_batch_partial_completion_impact.csv")),
