"""Health checks for reviewed replacement worklist plan artifacts."""
...
from quant_replay_system.reviewed_replacement_worklist_plan import PLAN_COLUMNS
from quant_replay_system.reviewed_replacement_worklist_plan_index import (
    build_reviewed_replacement_worklist_plan_index,
)


HEALTH_COLUMNS = ["replacement_plan_id", "status", "severity", "issue_code", "message", "artifact_path"]


@dataclass(frozen=True)
class ReviewedReplacementWorklistPlanHealthResult:
    status: str
...
def check_reviewed_replacement_worklist_plan_health(
    *,
    root: str | Path = "outputs/reports/reviewed_replacement_worklist_plan",
    index_path: str | Path | None = None,
    output_dir: str | Path = "outputs/reports/reviewed_replacement_worklist_plan/health",
) -> ReviewedReplacementWorklistPlanHealthResult:
    if index_path:
        index_frame = read_csv_preserve_symbol_columns(index_path, keep_default_na=False)
    else:
        index_frame = build_reviewed_replacement_worklist_plan_index(root=root).index_frame
    ...
    paths = {
        ...
        "reviewed_replacement_worklist_plan_health_csv": artifact_dir / "reviewed_replacement_worklist_plan_health.csv",
        "reviewed_replacement_worklist_plan_health_report": artifact_dir / "reviewed_replacement_worklist_plan_health_report.md",
        "metadata": artifact_dir / "metadata.json",
    }
    result = ReviewedReplacementWorklistPlanHealthResult(
        status=status,
...
def _issues_for_row(row: dict[str, Any]) -> list[dict[str, Any]]:
    issues: list[dict[str, Any]] = []
    plan_id = _text(row.get("replacement_plan_id"))
    ...
    if plan_csv_path.exists():
        plan_frame = read_csv_preserve_symbol_columns(plan_csv_path, keep_default_na=False)
        missing = sorted(set(PLAN_COLUMNS) - set(plan_frame.columns))
        if missing:
            issues.append(_issue(plan_id, "ERROR", "MISSING_REQUIRED_COLUMNS", ", ".join(missing), plan_csv_path))
        if "active_worklist_mutated" in plan_frame and plan_frame["active_worklist_mutated"].map(_to_bool).any():
            issues.append(_issue(plan_id, "ERROR", "ACTIVE_WORKLIST_MUTATION_DETECTED", "Plan rows claim active worklist mutation.", plan_csv_path))
        if "no_approval_applied" in plan_frame and (~plan_frame["no_approval_applied"].map(_to_bool)).any():
            issues.append(_issue(plan_id, "ERROR", "APPROVAL_APPLIED_DETECTED", "Plan rows indicate approval was applied.", plan_csv_path))
        if "no_rejection_applied" in plan_frame and (~plan_frame["no_rejection_applied"].map(_to_bool)).any():
            issues.append(_issue(plan_id, "ERROR", "REJECTION_APPLIED_DETECTED", "Plan rows indicate rejection was applied.", plan_csv_path))
    if _to_bool(row.get("active_worklist_mutated")):
        issues.append(_issue(plan_id, "ERROR", "ACTIVE_WORKLIST_MUTATION_DETECTED", "Metadata claims active worklist mutation.", metadata_path))
    false_flag_issues = {
        ...
        "replacement_plan_only": "PLAN_ONLY_FLAG_MISSING",
    }
...
def _write(result: ReviewedReplacementWorklistPlanHealthResult) -> None:
    paths = result.artifact_paths
    paths["artifact_dir"].mkdir(parents=True, exist_ok=True)
    result.health_frame.to_csv(paths["reviewed_replacement_worklist_plan_health_csv"], index=False)
    paths["reviewed_replacement_worklist_plan_health_report"].write_text(
        "\n".join(
            [
