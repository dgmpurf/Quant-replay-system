"""Index for reviewer material evidence fill guidance artifacts."""
...
class ReviewerMaterialEvidenceFillGuidanceIndexResult:
    artifact_count: int
...
def build_reviewer_material_evidence_fill_guidance_index(
    *,
    root: str | Path = "outputs/reports/reviewer_material_evidence_fill_guidance",
    output_dir: str | Path = "outputs/reports/reviewer_material_evidence_fill_guidance/index",
) -> ReviewerMaterialEvidenceFillGuidanceIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows))
    paths = {
        "artifact_dir": Path(output_dir),
        "index_csv": Path(output_dir) / "reviewer_material_evidence_fill_guidance_index.csv",
        "index_report": Path(output_dir) / "reviewer_material_evidence_fill_guidance_index_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    result = ReviewerMaterialEvidenceFillGuidanceIndexResult(
    ...
    write_reviewer_material_evidence_fill_guidance_index(result)
    return result


def write_reviewer_material_evidence_fill_guidance_index(
    result: ReviewerMaterialEvidenceFillGuidanceIndexResult,
) -> dict[str, Path]:
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Reviewer material evidence fill guidance root does not exist: {root}"]
    ...
    for artifact_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        try:
            ...
            continue
        if _text(metadata.get("guidance_id")):
...
def _row_from_metadata(artifact_dir: Path, metadata_path: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "artifact_type": "REVIEWER_MATERIAL_EVIDENCE_FILL_GUIDANCE",
        "guidance_id": _text(metadata.get("guidance_id")) or artifact_dir.name,
        "status": _text(metadata.get("status")),
        "row_count": _to_int(metadata.get("row_count")),
        "reviewer_guidance_row_count": _to_int(metadata.get("reviewer_guidance_row_count")),
        "symbol_level_guidance_count": _to_int(metadata.get("symbol_level_guidance_count")),
        "date_specific_guidance_count": _to_int(metadata.get("date_specific_guidance_count")),
        "no_hit_acceptance_guidance_count": _to_int(metadata.get("no_hit_acceptance_guidance_count")),
        "survivorship_rationale_guidance_count": _to_int(metadata.get("survivorship_rationale_guidance_count")),
        "metadata_guidance_count": _to_int(metadata.get("metadata_guidance_count")),
        ...
        "guidance_only": _to_bool(metadata.get("guidance_only")),
        "report_path": str(Path(output_files.get("report") or artifact_dir / "report.md")),
        "guidance_csv_path": str(
            Path(output_files.get("guidance_csv") or artifact_dir / "reviewer_material_evidence_fill_guidance.csv")
        ),
        "safe_template_path": str(
            Path(
                output_files.get("reviewer_fill_template_safe_defaults")
                or artifact_dir / "reviewer_fill_template_safe_defaults.csv"
            )
        ),
