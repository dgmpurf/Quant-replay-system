"""Health view for reviewer no-hit acceptance fixture artifacts."""
...
STATUS_HEALTH_PASS = "REVIEWER_NO_HIT_ACCEPTANCE_FIXTURE_HEALTH_PASS_REPORT_ONLY"
STATUS_HEALTH_FAIL = "REVIEWER_NO_HIT_ACCEPTANCE_FIXTURE_HEALTH_FAIL_UNSAFE"
...
class HistoricalReplayReviewerNoHitAcceptanceFixtureHealthResult:
    status: str
...
def check_historical_replay_reviewer_no_hit_acceptance_fixture_health(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path | None = None,
) -> HistoricalReplayReviewerNoHitAcceptanceFixtureHealthResult:
    root_path = Path(root)
    ...
    result = HistoricalReplayReviewerNoHitAcceptanceFixtureHealthResult(
        status=status,
        ...
        warnings=[] if status == STATUS_HEALTH_PASS else [f"Reviewer no-hit acceptance fixture health is {status}."],
    )
    _write(result)
    return result
...
def _issues_for_artifact_dir(artifact_dir: Path) -> list[dict[str, str]]:
    ...
    rows = _read_csv(paths["reviewer_no_hit_acceptance_rows"]) if paths["reviewer_no_hit_acceptance_rows"].exists() else []
    ...
    issues.extend(_row_issues(run_id, rows, paths["reviewer_no_hit_acceptance_rows"]))
...
def _metadata_issues(
    ...
    checks = [
        ...
        ("no_hit_row_count", 9, "NO_HIT_COUNT_MISMATCH"),
    ...
    if rows and len(rows) != 9:
        issues.append(_issue(run_id, "ERROR", "NO_HIT_COUNT_MISMATCH", "No-hit row count must be 9.", path))
...
def _safety_issues(run_id: str, safety: dict[str, Any], path: Path) -> list[dict[str, str]]:
    ...
    if _to_bool(safety.get("no_hit_context_accepted")):
        issues.append(_issue(run_id, "ERROR", "NO_HIT_CONTEXT_ACCEPTED_UNSAFE", "No-hit context must remain unaccepted.", path))
    return issues
...
def _row_issues(run_id: str, rows: list[dict[str, str]], path: Path) -> list[dict[str, str]]:
    ...
    for row in rows:
        ...
        if _text(row.get("no_hit_acceptance_status")) != "not_accepted" or _to_bool(row.get("no_hit_context_accepted")):
            issues.append(_issue(run_id, "ERROR", "NO_HIT_CONTEXT_ACCEPTED_UNSAFE", "No-hit rows must remain not accepted.", path))
        if not _text(row.get("blocker_reason")):
            issues.append(_issue(run_id, "ERROR", "NO_HIT_BLOCKERS_MISSING", "No-hit rows must keep blocker reasons visible.", path))
        if _text(row.get("reviewer_private_identity_disclosed")) != "no":
            issues.append(_issue(run_id, "ERROR", "PRIVATE_REVIEWER_IDENTITY_DISCLOSED", "Reviewer private identity must not be disclosed.", path))
        for field in [
            ...
            if _to_bool(row.get(field)):
                issues.append(_issue(run_id, "ERROR", "FORBIDDEN_NO_HIT_DOWNSTREAM_USE", f"{field} must remain false.", path))
    return issues
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        ...
        "health_csv": output_dir / "historical_replay_reviewer_no_hit_acceptance_fixture_health.csv",
        "health_md": output_dir / "historical_replay_reviewer_no_hit_acceptance_fixture_health.md",
        "metadata_json": output_dir / "metadata.json",
    }


def _write(result: HistoricalReplayReviewerNoHitAcceptanceFixtureHealthResult) -> None:
...
def _render_markdown(result: HistoricalReplayReviewerNoHitAcceptanceFixtureHealthResult) -> str:
    return "\n".join(
        [
            ...
            "- Report-only health; no no-hit evidence acceptance, PIT approval, replay, buy-review, trading, or protected data-write behavior is authorized.",
            "",
        ]
    )
