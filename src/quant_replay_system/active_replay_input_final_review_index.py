"""Index report-only active replay input final-review artifacts."""
...
DEFAULT_ROOT = Path("outputs/reports/manual_diagnostics/active_replay_input_final_review_v0_1")
...
INDEX_COLUMNS = [
    ...
    "generated_at",
    "artifact_path",
    "active_ready_artifact_path",
    "active_ready_health_artifact_path",
    "active_ready_status_artifact_path",
    "final_review_package_manifest_path",
    "final_review_authority_manifest_path",
    "final_review_attestation_manifest_path",
    "pit_source_evidence_attachment_bundle_path",
    "taxonomy_attachment_bundle_path",
    ...
    "no_message_sent",
    "package_gate_count",
    "passed_package_gate_count",
    "blocked_package_gate_count",
    "lineage_gate_count",
...
class ActiveReplayInputFinalReviewIndexResult:
    artifact_count: int
...
def build_active_replay_input_final_review_index(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path = DEFAULT_OUTPUT_DIR,
) -> ActiveReplayInputFinalReviewIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows, dtype=object))
    paths = {
        "artifact_dir": Path(output_dir),
        "index_csv": Path(output_dir) / "active_replay_input_final_review_index.csv",
        "index_report": Path(output_dir) / "active_replay_input_final_review_index_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    result = ActiveReplayInputFinalReviewIndexResult(
    ...
    write_active_replay_input_final_review_index(result)
    return result


def write_active_replay_input_final_review_index(
    result: ActiveReplayInputFinalReviewIndexResult,
) -> dict[str, Path]:
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Final-review root does not exist: {root}"]
    rows: list[dict[str, Any]] = []
    warnings: list[str] = []
    for artifact_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        if artifact_dir.name in {"index", "health", "status"} or artifact_dir.name.startswith("_"):
            continue
        metadata_path = artifact_dir / "final_review_metadata.json"
        if not metadata_path.exists():
            continue
        try:
            metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            warnings.append(f"Could not read final-review metadata {metadata_path}: {exc}")
            continue
        if _text(metadata.get("final_review_run_id")):
...
def _row_from_metadata(artifact_dir: Path, metadata_path: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "final_review_run_id": _text(metadata.get("final_review_run_id")),
        ...
        "active_ready_artifact_path": _text(metadata.get("active_ready_artifact_path")),
        ...
        "final_review_package_manifest_path": _text(metadata.get("final_review_package_manifest_path")),
        "final_review_authority_manifest_path": _text(metadata.get("final_review_authority_manifest_path")),
        "final_review_attestation_manifest_path": _text(metadata.get("final_review_attestation_manifest_path")),
        "pit_source_evidence_attachment_bundle_path": _text(metadata.get("pit_source_evidence_attachment_bundle_path")),
        "taxonomy_attachment_bundle_path": _text(metadata.get("taxonomy_attachment_bundle_path")),
        ...
        "ready_for_emission_review": _to_bool(metadata.get("ready_for_emission_review")),
        ...
        "package_gate_count": _to_int(metadata.get("package_gate_count")),
        "passed_package_gate_count": _to_int(metadata.get("passed_package_gate_count")),
        "blocked_package_gate_count": _to_int(metadata.get("blocked_package_gate_count")),
        ...
        "final_review_report_path": _text(artifact_paths.get("final_review_report"))
        or str(artifact_dir / "final_review_report.md"),
...
__all__ = [
    ...
    "write_active_replay_input_final_review_index",
]
