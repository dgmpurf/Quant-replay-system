"""Index report-only active replay input ready-decision artifacts."""
...
DEFAULT_ROOT = Path("outputs/reports/manual_diagnostics/active_replay_input_ready_decision_v0_1")
...
class ActiveReplayInputReadyDecisionIndexResult:
    artifact_count: int
...
def build_active_replay_input_ready_decision_index(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path = DEFAULT_OUTPUT_DIR,
) -> ActiveReplayInputReadyDecisionIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows, dtype=object))
    paths = {
        "artifact_dir": Path(output_dir),
        "index_csv": Path(output_dir) / "active_replay_input_ready_decision_index.csv",
        "index_report": Path(output_dir) / "active_replay_input_ready_decision_index_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    result = ActiveReplayInputReadyDecisionIndexResult(
    ...
    write_active_replay_input_ready_decision_index(result)
    return result


def write_active_replay_input_ready_decision_index(
    result: ActiveReplayInputReadyDecisionIndexResult,
) -> dict[str, Path]:
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Ready-decision root does not exist: {root}"]
    rows: list[dict[str, Any]] = []
    warnings: list[str] = []
    for artifact_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        if artifact_dir.name in {"index", "health", "status"} or artifact_dir.name.startswith("_"):
            continue
        metadata_path = artifact_dir / "decision_metadata.json"
        if not metadata_path.exists():
            continue
        try:
            metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            warnings.append(f"Could not read ready-decision metadata {metadata_path}: {exc}")
            continue
        if _text(metadata.get("decision_run_id")):
...
def _row_from_metadata(artifact_dir: Path, metadata_path: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "decision_run_id": _text(metadata.get("decision_run_id")),
        ...
        "emission_artifact_path": _text(metadata.get("emission_artifact_path")),
        "emission_health_artifact_path": _text(metadata.get("emission_health_artifact_path")),
        "emission_status_artifact_path": _text(metadata.get("emission_status_artifact_path")),
        "emission_acceptance_audit_path": _text(metadata.get("emission_acceptance_audit_path")),
        "decision_request_manifest_path": _text(metadata.get("decision_request_manifest_path")),
        "decision_authority_manifest_path": _text(metadata.get("decision_authority_manifest_path")),
        "decision_attestation_manifest_path": _text(metadata.get("decision_attestation_manifest_path")),
        ...
        "ready_for_active_replay_input_ready_decision": _to_bool(
            metadata.get("ready_for_active_replay_input_ready_decision")
        ),
        ...
        "decision_report_path": _text(artifact_paths.get("decision_report")) or str(artifact_dir / "decision_report.md"),
...
__all__ = [
    ...
    "write_active_replay_input_ready_decision_index",
]
