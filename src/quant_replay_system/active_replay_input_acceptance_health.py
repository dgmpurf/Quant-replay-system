"""Health checks for report-only active replay input acceptance artifacts."""
...
from quant_replay_system.active_replay_input_acceptance import (
    ACCEPTANCE_ATTESTATION_BLOCKED,
    ACCEPTANCE_AUTHORITY_BLOCKED,
    ACCEPTANCE_EVIDENCE_BLOCKED,
    ACCEPTANCE_INPUT_FOUND,
    ACCEPTANCE_LEAKAGE_BLOCKED,
    ACCEPTANCE_LINEAGE_BLOCKED,
    ACCEPTANCE_PIT_BLOCKED,
    ACCEPTANCE_READY_FOR_ACTIVE_READY_REVIEW,
    ACCEPTANCE_RED_TEAM_BLOCKED,
    ACCEPTANCE_REVIEW_BLOCKED,
    ACCEPTANCE_SECOND_REVIEW_BLOCKED,
    ACCEPTANCE_SIDE_EFFECT_BLOCKED,
    ACCEPTANCE_SOURCE_BLOCKED,
    NO_ACCEPTANCE_INPUT,
)
...
HEALTH_COLUMNS = ["acceptance_run_id", "severity", "issue_code", "message", "artifact_path"]


@dataclass(frozen=True)
class ActiveReplayInputAcceptanceHealthResult:
    status: str
...
def check_active_replay_input_acceptance_health(
    *,
    root: str | Path = "outputs/reports/manual_diagnostics/active_replay_input_acceptance_v0_1",
    output_dir: str | Path = "outputs/reports/manual_diagnostics/active_replay_input_acceptance_v0_1/health",
) -> ActiveReplayInputAcceptanceHealthResult:
    index = build_active_replay_input_acceptance_index(root=root, output_dir=Path(output_dir).parent / "index")
    issues: list[dict[str, Any]] = []
    if index.index_frame.empty:
        issues.append(_issue("", "ERROR", "NO_ACCEPTANCE_ARTIFACT_FOUND", "No acceptance artifacts found.", root))
    else:
    ...
    paths = {
        ...
        "health_csv": Path(output_dir) / "active_replay_input_acceptance_health.csv",
        "health_report": Path(output_dir) / "active_replay_input_acceptance_health_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    result = ActiveReplayInputAcceptanceHealthResult(
        status=status,
...
def _issues_for_latest(row: dict[str, Any]) -> list[dict[str, Any]]:
    acceptance_run_id = _text(row.get("acceptance_run_id"))
    ...
    required_paths = [
        ...
        (Path(_text(row.get("acceptance_report_path"))), "MISSING_ACCEPTANCE_REPORT"),
        (artifact_path / "acceptance_precondition_results.csv", "MISSING_PRECONDITION_RESULTS"),
        (artifact_path / "reviewer_authority_results.csv", "MISSING_REVIEWER_AUTHORITY_RESULTS"),
        (artifact_path / "manual_attestation_results.csv", "MISSING_MANUAL_ATTESTATION_RESULTS"),
        (artifact_path / "second_review_results.csv", "MISSING_SECOND_REVIEW_RESULTS"),
        (artifact_path / "red_team_review_results.csv", "MISSING_RED_TEAM_REVIEW_RESULTS"),
        (artifact_path / "lineage_acceptance_results.csv", "MISSING_LINEAGE_ACCEPTANCE_RESULTS"),
        (artifact_path / "pit_acceptance_results.csv", "MISSING_PIT_ACCEPTANCE_RESULTS"),
        (artifact_path / "source_acceptance_results.csv", "MISSING_SOURCE_ACCEPTANCE_RESULTS"),
        (artifact_path / "leakage_acceptance_results.csv", "MISSING_LEAKAGE_ACCEPTANCE_RESULTS"),
        (artifact_path / "side_effect_acceptance_results.csv", "MISSING_SIDE_EFFECT_ACCEPTANCE_RESULTS"),
    ...
    for path, code in required_paths:
        if not _text(path) or not path.exists():
            issues.append(_issue(acceptance_run_id, "ERROR", code, f"Required acceptance artifact missing: {path}", path))

    status = _text(row.get("status"))
    if status == "ACTIVE_REPLAY_INPUT_READY":
        issues.append(_issue(acceptance_run_id, "ERROR", "ACTIVE_REPLAY_INPUT_READY_UNEXPECTED", "ACTIVE_REPLAY_INPUT_READY must not be emitted.", metadata_path))
    elif status not in ALLOWED_STATUSES:
        issues.append(_issue(acceptance_run_id, "ERROR", "UNKNOWN_ACCEPTANCE_STATUS", f"Unknown acceptance status: {status}", metadata_path))
    ...
    for field, code in false_expected:
        if _to_bool(row.get(field)):
            issues.append(_issue(acceptance_run_id, "ERROR", code, f"Unsafe false-expected field is true: {field}", metadata_path))

    for field in ["report_only", "diagnostic_only"]:
        if not _to_bool(row.get(field)):
            issues.append(_issue(acceptance_run_id, "ERROR", "UNSAFE_REPORT_ONLY_FLAGS", f"Safety flag is missing or false: {field}", metadata_path))
    for field in ["no_live_trading", "no_broker_api", "no_order_placement", "no_message_sent"]:
        if not _to_bool(row.get(field)):
            issues.append(_issue(acceptance_run_id, "ERROR", "UNSAFE_TRADING_FLAGS", f"Trading safety flag is missing or false: {field}", metadata_path))
    ...
    if _to_int(row.get("overclaim_guard_total_count")) <= 0 or _to_int(row.get("overclaim_guard_pass_count")) != _to_int(row.get("overclaim_guard_total_count")):
        issues.append(_issue(acceptance_run_id, "ERROR", "OVERCLAIM_GUARD_FAILED", "Overclaim guard counts do not fully pass.", metadata_path))
    ...
    if guard_path.exists():
        try:
            ...
            if not guards.empty and not guards["passed"].map(_to_bool).all():
                issues.append(_issue(acceptance_run_id, "ERROR", "OVERCLAIM_GUARD_FAILED", "At least one overclaim guard failed.", guard_path))
        except Exception as exc:  # pragma: no cover
            issues.append(_issue(acceptance_run_id, "ERROR", "OVERCLAIM_GUARD_FAILED", f"Could not parse overclaim guards: {exc}", guard_path))

    if not _safe_diagnostics_path(artifact_path) or _unsafe_path(artifact_path):
        issues.append(_issue(acceptance_run_id, "ERROR", "UNSAFE_OUTPUT_PATH", f"Unsafe acceptance artifact path: {artifact_path}", artifact_path))
    return issues


def _write(result: ActiveReplayInputAcceptanceHealthResult) -> None:
...
def _issue(acceptance_run_id: str, severity: str, issue_code: str, message: str, artifact_path: str | Path) -> dict[str, Any]:
    return {
