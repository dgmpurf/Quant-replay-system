"""Index view for Tiny PIT reviewed package fixture artifacts."""
...
NO_TINY_PIT_REVIEWED_PACKAGE_FIXTURE = "NO_TINY_PIT_REVIEWED_PACKAGE_FIXTURE"
...
class TinyPitReviewedPackageFixtureIndexResult:
    artifact_count: int
...
def build_tiny_pit_reviewed_package_fixture_index(
    *,
    root: str | Path = "outputs/reports/manual_diagnostics/tiny_pit_reviewed_package_fixture_v0_1",
    output_dir: str | Path = "outputs/reports/manual_diagnostics/tiny_pit_reviewed_package_fixture_v0_1/index",
) -> TinyPitReviewedPackageFixtureIndexResult:
    rows, warnings = _scan_rows(Path(root))
    frame = _finalize(pd.DataFrame(rows, dtype=object))
    latest = _latest_row(frame)
    paths = _paths(output_dir)
    result = TinyPitReviewedPackageFixtureIndexResult(
        artifact_count=len(frame),
        latest_fixture_id=_text(latest.get("fixture_id")),
        latest_status=_text(latest.get("status")) or "NO_PACKAGE_FIXTURE",
        latest_health_status=_text(latest.get("health_status")) or "WARN",
        latest_workflow_stage=_text(latest.get("workflow_stage")) or NO_TINY_PIT_REVIEWED_PACKAGE_FIXTURE,
...
def _scan_rows(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Tiny PIT reviewed package fixture root does not exist: {root}"]
    ...
    for artifact_dir in _candidate_dirs(root):
        ...
        try:
            ...
            warnings.append(f"Could not read Tiny PIT reviewed package fixture metadata {metadata_path}: {exc}")
            continue
...
def _row_from_metadata(artifact_dir: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "pass_count": _to_int(metadata.get("pass_count")),
        ...
        "report_path": _text(metadata.get("report_path")) or str(artifact_dir / "tiny_pit_reviewed_package_fixture_report.md"),
...
def _write(result: TinyPitReviewedPackageFixtureIndexResult) -> None:
...
def _paths(output_dir: str | Path) -> dict[str, Path]:
    ...
    return {
        ...
        "index_csv": artifact_dir / "tiny_pit_reviewed_package_fixture_index.csv",
