"""Health checks for reviewed replacement worklist acceptance artifacts."""
...
HEALTH_COLUMNS = ["acceptance_id", "status", "severity", "issue_code", "message", "artifact_path"]


@dataclass(frozen=True)
class ReviewedReplacementWorklistAcceptanceHealthResult:
    status: str
...
def check_reviewed_replacement_worklist_acceptance_health(
    *,
    root: str | Path = "outputs/reports/reviewed_replacement_worklist_acceptance",
    index_path: str | Path | None = None,
    output_dir: str | Path = "outputs/reports/reviewed_replacement_worklist_acceptance/health",
) -> ReviewedReplacementWorklistAcceptanceHealthResult:
    if index_path:
        index_frame = read_csv_preserve_symbol_columns(index_path, keep_default_na=False)
    else:
        index_frame = build_reviewed_replacement_worklist_acceptance_index(root=root).index_frame
    ...
    paths = {
        ...
        "reviewed_replacement_worklist_acceptance_health_csv": artifact_dir
        / "reviewed_replacement_worklist_acceptance_health.csv",
        "reviewed_replacement_worklist_acceptance_health_report": artifact_dir
        / "reviewed_replacement_worklist_acceptance_health_report.md",
        "metadata": artifact_dir / "metadata.json",
    }
    result = ReviewedReplacementWorklistAcceptanceHealthResult(
        status=status,
...
def _issues_for_row(row: dict[str, Any]) -> list[dict[str, Any]]:
    issues: list[dict[str, Any]] = []
    acceptance_id = _text(row.get("acceptance_id"))
    metadata_path = Path(_text(row.get("metadata_path")))
    acceptance_csv_path = Path(_text(row.get("acceptance_csv_path")))
    report_path = Path(_text(row.get("report_path")))
    for path, code in [
        (metadata_path, "MISSING_METADATA"),
        (acceptance_csv_path, "MISSING_ACCEPTANCE_CSV"),
        ...
        if not _text(path) or not path.exists():
            issues.append(_issue(acceptance_id, "ERROR", code, f"Required artifact is missing: {path}", path))
    if acceptance_csv_path.exists():
        frame = read_csv_preserve_symbol_columns(acceptance_csv_path, keep_default_na=False)
        missing = sorted(set(ACCEPTANCE_COLUMNS) - set(frame.columns))
        if missing:
            issues.append(_issue(acceptance_id, "ERROR", "MISSING_REQUIRED_COLUMNS", ", ".join(missing), acceptance_csv_path))
        if "active_worklist_mutated" in frame and frame["active_worklist_mutated"].map(_to_bool).any():
            issues.append(_issue(acceptance_id, "ERROR", "ACTIVE_WORKLIST_MUTATION_DETECTED", "Acceptance rows claim active worklist mutation.", acceptance_csv_path))
        if "no_approval_applied" in frame and (~frame["no_approval_applied"].map(_to_bool)).any():
            issues.append(_issue(acceptance_id, "ERROR", "APPROVAL_APPLIED_DETECTED", "Acceptance rows indicate approval was applied.", acceptance_csv_path))
        if "no_rejection_applied" in frame and (~frame["no_rejection_applied"].map(_to_bool)).any():
            issues.append(_issue(acceptance_id, "ERROR", "REJECTION_APPLIED_DETECTED", "Acceptance rows indicate rejection was applied.", acceptance_csv_path))
        if "valid_for_signal_date" in frame and frame["valid_for_signal_date"].map(_to_bool).any():
            issues.append(_issue(acceptance_id, "ERROR", "PIT_ROW_VALIDATION_DETECTED", "Acceptance rows claim valid PIT rows.", acceptance_csv_path))
    if not _to_bool(row.get("acceptance_acknowledged")):
        issues.append(_issue(acceptance_id, "ERROR", "ACCEPTANCE_ACKNOWLEDGEMENT_MISSING", "Acceptance acknowledgement flag is missing.", metadata_path))
    if not _text(row.get("accepted_by")) or not _text(row.get("accepted_at")) or not _text(row.get("acceptance_reason")):
        issues.append(_issue(acceptance_id, "ERROR", "ACCEPTANCE_METADATA_MISSING", "accepted_by, accepted_at, and acceptance_reason are required.", metadata_path))
    if _to_bool(row.get("active_worklist_mutated")):
        issues.append(_issue(acceptance_id, "ERROR", "ACTIVE_WORKLIST_MUTATION_DETECTED", "Metadata claims active worklist mutation.", metadata_path))
    false_flag_issues = {
        ...
        "acceptance_only": "ACCEPTANCE_ONLY_FLAG_MISSING",
    }
    for field, code in false_flag_issues.items():
        if not _to_bool(row.get(field)):
            issues.append(_issue(acceptance_id, "ERROR", code, f"Safety flag {field} is not true.", metadata_path))
    return issues


def _write(result: ReviewedReplacementWorklistAcceptanceHealthResult) -> None:
    paths = result.artifact_paths
    paths["artifact_dir"].mkdir(parents=True, exist_ok=True)
    result.health_frame.to_csv(paths["reviewed_replacement_worklist_acceptance_health_csv"], index=False)
    paths["reviewed_replacement_worklist_acceptance_health_report"].write_text(
        "\n".join(
            [
...
def _issue(acceptance_id: str, severity: str, code: str, message: str, path: Path) -> dict[str, Any]:
    return {
