"""Local-only index for reviewed replacement worklist activation artifacts."""
...
class ReviewedReplacementWorklistActivationIndexResult:
    artifact_count: int
...
def build_reviewed_replacement_worklist_activation_index(
    *,
    root: str | Path = "outputs/reports/reviewed_replacement_worklist_activation",
    output_dir: str | Path = "outputs/reports/reviewed_replacement_worklist_activation/index",
    include_missing_metadata: bool = False,
) -> ReviewedReplacementWorklistActivationIndexResult:
    ...
    paths = {
        ...
        "reviewed_replacement_worklist_activation_index_csv": artifact_dir
        / "reviewed_replacement_worklist_activation_index.csv",
        "reviewed_replacement_worklist_activation_index_report": artifact_dir
        / "reviewed_replacement_worklist_activation_index_report.md",
        "metadata": artifact_dir / "metadata.json",
    }
    result = ReviewedReplacementWorklistActivationIndexResult(
    ...
    write_reviewed_replacement_worklist_activation_index(result)
    return result


def scan_reviewed_replacement_worklist_activation_artifacts(
    root: str | Path = "outputs/reports/reviewed_replacement_worklist_activation",
) -> pd.DataFrame:
...
def write_reviewed_replacement_worklist_activation_index(
    result: ReviewedReplacementWorklistActivationIndexResult,
) -> dict[str, Path]:
    paths = result.artifact_paths
    paths["artifact_dir"].mkdir(parents=True, exist_ok=True)
    result.index_frame.to_csv(paths["reviewed_replacement_worklist_activation_index_csv"], index=False)
    metadata = {
    ...
    paths["reviewed_replacement_worklist_activation_index_report"].write_text(
        "\n".join(
            [
...
def _scan_rows(root: Path, *, include_missing_metadata: bool) -> tuple[list[dict[str, Any]], list[str]]:
    if not root.exists():
        return [], [f"Reviewed replacement worklist activation root does not exist: {root}"]
    ...
    for artifact_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        try:
            ...
            continue
        if not _text(metadata.get("activation_id")):
            continue
...
def _row_from_metadata(artifact_dir: Path, metadata_path: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    activation_csv = Path(
        output_files.get("reviewed_replacement_worklist_activation")
        or artifact_dir / "reviewed_replacement_worklist_activation.csv"
    )
    frame = _read_csv(activation_csv)
    return {
        "artifact_type": "REVIEWED_REPLACEMENT_WORKLIST_ACTIVATION",
        "activation_id": _text(metadata.get("activation_id")) or artifact_dir.name,
        ...
        "activated_by": _text(metadata.get("activated_by")),
        "activated_at": _text(metadata.get("activated_at")),
        "activation_reason": _text(metadata.get("activation_reason")),
        "activation_acknowledged": _to_bool(metadata.get("activation_acknowledged")),
        ...
        "activation_only": _to_bool(metadata.get("activation_only")),
        "report_path": str(Path(output_files.get("report") or artifact_dir / "report.md")),
        "activation_csv_path": str(activation_csv),
        "stock_core_worklist_path": str(Path(output_files.get("activated_replacement_worklist_stock_core") or artifact_dir / "activated_replacement_worklist_stock_core.csv")),
        "etf_core_worklist_path": str(Path(output_files.get("activated_replacement_worklist_etf_core") or artifact_dir / "activated_replacement_worklist_etf_core.csv")),
        "mixed_demo_core_worklist_path": str(Path(output_files.get("activated_replacement_worklist_mixed_demo_core") or artifact_dir / "activated_replacement_worklist_mixed_demo_core.csv")),
        "stock_core_template_path": str(Path(output_files.get("activated_update_template_stock_core") or artifact_dir / "activated_update_template_stock_core.csv")),
        "etf_core_template_path": str(Path(output_files.get("activated_update_template_etf_core") or artifact_dir / "activated_update_template_etf_core.csv")),
        "mixed_demo_core_template_path": str(Path(output_files.get("activated_update_template_mixed_demo_core") or artifact_dir / "activated_update_template_mixed_demo_core.csv")),
...
def _missing_row(artifact_dir: Path, *, status: str = "MISSING_METADATA") -> dict[str, Any]:
    ...
    row.update({"artifact_type": "REVIEWED_REPLACEMENT_WORKLIST_ACTIVATION", "activation_id": artifact_dir.name, "status": status})
    return row
...
def _finalize(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    return frame.loc[:, INDEX_COLUMNS].sort_values(["created_at", "activation_id"]).reset_index(drop=True)
