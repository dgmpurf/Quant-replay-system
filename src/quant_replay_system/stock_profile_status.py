"""Status summary for report-only stock profile phase 1 artifacts."""
...
from quant_replay_system.stock_profile_index import _text, _to_bool, _to_int


DEFAULT_OUTPUT_DIR = DEFAULT_ROOT / "status"
NO_STOCK_PROFILE_ARTIFACT_FOUND = "NO_STOCK_PROFILE_ARTIFACT_FOUND"
STOCK_PROFILE_HEALTH_FAILED = "STOCK_PROFILE_HEALTH_FAILED"
...
class StockProfileStatusResult:
...
def run_stock_profile_status(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path = DEFAULT_OUTPUT_DIR,
) -> StockProfileStatusResult:
    sibling_root = Path(output_dir).parent
    index = build_stock_profile_index(root=root, output_dir=sibling_root / "index")
    health = check_stock_profile_health(root=root, output_dir=sibling_root / "health")
    if index.index_frame.empty:
        ...
        latest = frame.sort_values(["created_at", "_status_priority", "stock_profile_run_id"]).iloc[-1].to_dict()
...
def _result_from_latest(
    ...
    stage = STOCK_PROFILE_HEALTH_FAILED if health_status == "FAIL" else _text(latest.get("workflow_stage")) or status
    summary = {
        "latest_stock_profile_run_id": _text(latest.get("stock_profile_run_id")),
        "status": status,
        "health_status": health_status,
        "workflow_stage": stage,
        "ready_for_stock_profile_phase1": _to_bool(latest.get("ready_for_stock_profile_phase1")),
        "stock_profile_phase1_executed": _to_bool(latest.get("stock_profile_phase1_executed")),
        "stock_profile_phase1_report_only_artifacts_created": _to_bool(latest.get("stock_profile_phase1_report_only_artifacts_created")),
        "stock_profile_metadata_created": _to_bool(latest.get("stock_profile_metadata_created")),
        "stock_profile_input_index_created": _to_bool(latest.get("stock_profile_input_index_created")),
        "stock_profile_lineage_matrix_created": _to_bool(latest.get("stock_profile_lineage_matrix_created")),
        "stock_profile_factor_coverage_summary_created": _to_bool(latest.get("stock_profile_factor_coverage_summary_created")),
        "stock_profile_symbol_coverage_created": _to_bool(latest.get("stock_profile_symbol_coverage_created")),
        "stock_profile_market_regime_coverage_created": _to_bool(latest.get("stock_profile_market_regime_coverage_created")),
        "stock_profile_metric_summary_created": _to_bool(latest.get("stock_profile_metric_summary_created")),
        "stock_profile_limitations_created": _to_bool(latest.get("stock_profile_limitations_created")),
        "stock_profile_overfit_warnings_created": _to_bool(latest.get("stock_profile_overfit_warnings_created")),
        "stock_profile_safety_flags_created": _to_bool(latest.get("stock_profile_safety_flags_created")),
        ...
        **{field: _to_bool(latest.get(field)) for field in DOWNSTREAM_FALSE_FIELDS},
        ...
        "report_path": _text(latest.get("stock_profile_limitations_path")),
...
def _no_artifact_result(
    ...
    summary = {
        ...
        "status": "MISSING",
        "health_status": health_status,
        "workflow_stage": NO_STOCK_PROFILE_ARTIFACT_FOUND,
        "ready_for_stock_profile_phase1": False,
        "stock_profile_phase1_executed": False,
        "stock_profile_phase1_report_only_artifacts_created": False,
        "stock_profile_metadata_created": False,
        "stock_profile_input_index_created": False,
        "stock_profile_lineage_matrix_created": False,
        "stock_profile_factor_coverage_summary_created": False,
        "stock_profile_symbol_coverage_created": False,
        "stock_profile_market_regime_coverage_created": False,
        "stock_profile_metric_summary_created": False,
        "stock_profile_limitations_created": False,
        "stock_profile_overfit_warnings_created": False,
        "stock_profile_safety_flags_created": False,
        "source_active_model_run_id": "",
        "source_active_model_status": "",
        "source_active_model_health_status": "",
        "source_model_workflow_run_id": "",
        ...
        **{field: False for field in DOWNSTREAM_FALSE_FIELDS},
        ...
        "report_path": "",
        "safety_statement": _safety_statement(),
        "next_action": "Create or provide report-only stock-profile artifacts before checking status.",
    }
    return _result(summary, output_dir, root, [])
...
def _result(
    ...
    paths = {
        ...
        "status_csv": Path(output_dir) / "stock_profile_status.csv",
        "status_report": Path(output_dir) / "stock_profile_status_report.md",
        "metadata": Path(output_dir) / "metadata.json",
    }
    return StockProfileStatusResult(
        summary_frame=frame,
...
def _write(result: StockProfileStatusResult) -> None:
    ...
    paths["status_report"].write_text(
        "\n".join(
            [
                ...
                "",
                f"- latest_stock_profile_run_id: {result.latest_stock_profile_run_id}",
                f"- status: {result.status}",
                f"- health_status: {result.health_status}",
                f"- workflow_stage: {result.workflow_stage}",
                f"- stock_profile_phase1_report_only_artifacts_created: {result.stock_profile_phase1_report_only_artifacts_created}",
...
def _safety_statement() -> str:
    return (
        "Stock Profile Phase 1 is report-only / research-governed artifact creation only. "
        "STOCK_PROFILE_PHASE1_REPORT_ONLY_ARTIFACTS_CREATED means metadata, lineage, coverage, "
        "metric summary, limitations, overfit warning, safety, and gate result artifacts only; it "
        "does not create active stock_profile, does not create real buy-review eligibility, does not "
        "apply paper approval, does not validate strategy performance, does not integrate "
        "current-candidates, does not build snapshots, does not mutate signal_semantics, does not "
        "create promoted model, does not create production model, does not create active thresholds, "
        "does not create advisory predictions, does not create active probabilities, and does not "
        "authorize broker/order/message/API/trading."
    )


def _next_action(stage: str, status: str) -> str:
    if stage == STOCK_PROFILE_HEALTH_FAILED:
        return "Resolve stock-profile artifact health failures before any future integration."
    ...
    if status == READY_FOR_STOCK_PROFILE_PHASE1:
        return "Rerun stock-profile with explicit allow only if report-only stock_profile phase-1 artifacts should be created."
    if status == NO_STOCK_PROFILE_INPUT:
        return "Provide exact approval and complete upstream active-model/model/training/metric/replay lineage before stock profile phase 1."
