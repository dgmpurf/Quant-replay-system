"""Health checks for PIT universe evidence review worklist artifacts."""
...
HEALTH_LIMITATIONS = [
    "Checks local PIT universe evidence review worklist artifacts only.",
...
class PitUniverseEvidenceReviewWorklistHealthPaths:
    artifact_dir: Path
    pit_universe_evidence_review_worklist_health_report: Path
    pit_universe_evidence_review_worklist_health_issues: Path
    pit_universe_evidence_review_worklist_health_summary: Path
    metadata: Path

    def as_dict(self) -> dict[str, Path]:
        return {
            "artifact_dir": self.artifact_dir,
            "pit_universe_evidence_review_worklist_health_report": (
                self.pit_universe_evidence_review_worklist_health_report
            ),
            "pit_universe_evidence_review_worklist_health_issues": (
                self.pit_universe_evidence_review_worklist_health_issues
            ),
            "pit_universe_evidence_review_worklist_health_summary": (
                self.pit_universe_evidence_review_worklist_health_summary
            ),
...
@dataclass(frozen=True)
class PitUniverseEvidenceReviewWorklistHealthResult:
    status: str
...
def check_pit_universe_evidence_review_worklist_health(
    *,
    index_df: pd.DataFrame | None = None,
    index_path: str | Path | None = None,
    root: str | Path = "outputs/reports/point_in_time_universe_evidence_review_worklist",
    output_dir: str | Path = "outputs/reports/point_in_time_universe_evidence_review_worklist/health",
) -> PitUniverseEvidenceReviewWorklistHealthResult:
    index_frame, index_source, load_issues = _load_index(index_df=index_df, index_path=index_path, root=root)
    health_frame = build_pit_universe_evidence_review_worklist_health_frame(index_frame)
    if load_issues:
        health_frame = _finalize_health_frame(pd.concat([pd.DataFrame(load_issues), health_frame], ignore_index=True))
    summary_frame = summarize_pit_universe_evidence_review_worklist_health(
        health_frame,
    ...
    paths = resolve_pit_universe_evidence_review_worklist_health_paths(output_dir, health_check_id)
    result = PitUniverseEvidenceReviewWorklistHealthResult(
        status=status,
        checked_artifact_count=len(index_frame),
    ...
    write_pit_universe_evidence_review_worklist_health_artifacts(result)
    return result


def build_pit_universe_evidence_review_worklist_health_frame(index_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in index_frame.to_dict("records"):
        ...
        worklist = _check_csv(
            row,
            Path(_string_or_empty(row.get("worklist_csv_path"))),
            issues,
            path_field="worklist_csv_path",
            missing_code="MISSING_WORKLIST_CSV",
            required_columns=WORKLIST_OUTPUT_COLUMNS,
        )
        update_template = _check_csv(
            row,
            Path(_string_or_empty(row.get("update_template_path"))),
            issues,
            path_field="update_template_path",
            missing_code="MISSING_UPDATE_TEMPLATE",
            required_columns=UPDATE_TEMPLATE_COLUMNS,
        )
        _check_report(row, Path(_string_or_empty(row.get("report_path"))), issues)
        if metadata is not None and worklist is not None and update_template is not None:
            _check_worklist_contract(row, metadata, worklist, update_template, issues)
    return _finalize_health_frame(pd.DataFrame(issues))


def summarize_pit_universe_evidence_review_worklist_health(
    health_frame: pd.DataFrame,
    *,
    checked_artifact_count: int,
) -> pd.DataFrame:
...
def resolve_pit_universe_evidence_review_worklist_health_paths(
    output_dir: str | Path,
    health_check_id: str,
) -> PitUniverseEvidenceReviewWorklistHealthPaths:
    artifact_dir = Path(output_dir) / health_check_id
    return PitUniverseEvidenceReviewWorklistHealthPaths(
        artifact_dir=artifact_dir,
        pit_universe_evidence_review_worklist_health_report=artifact_dir
        / "pit_universe_evidence_review_worklist_health_report.md",
        pit_universe_evidence_review_worklist_health_issues=artifact_dir
        / "pit_universe_evidence_review_worklist_health_issues.csv",
        pit_universe_evidence_review_worklist_health_summary=artifact_dir
        / "pit_universe_evidence_review_worklist_health_summary.csv",
        metadata=artifact_dir / "metadata.json",
    )


def write_pit_universe_evidence_review_worklist_health_artifacts(
    result: PitUniverseEvidenceReviewWorklistHealthResult,
) -> dict[str, Path]:
    paths = PitUniverseEvidenceReviewWorklistHealthPaths(**result.artifact_paths)
    paths.artifact_dir.mkdir(parents=True, exist_ok=True)
    result.health_frame.to_csv(paths.pit_universe_evidence_review_worklist_health_issues, index=False)
    result.summary_frame.to_csv(paths.pit_universe_evidence_review_worklist_health_summary, index=False)
    metadata = build_pit_universe_evidence_review_worklist_health_metadata(result, paths)
    paths.metadata.write_text(json.dumps(_json_safe(metadata), indent=2, sort_keys=True), encoding="utf-8")
    paths.pit_universe_evidence_review_worklist_health_report.write_text(
        render_pit_universe_evidence_review_worklist_health_report(result, metadata),
        encoding="utf-8",
    )
    return paths.as_dict()


def build_pit_universe_evidence_review_worklist_health_metadata(
    result: PitUniverseEvidenceReviewWorklistHealthResult,
    paths: PitUniverseEvidenceReviewWorklistHealthPaths,
) -> dict[str, Any]:
    return {
...
def render_pit_universe_evidence_review_worklist_health_report(
    result: PitUniverseEvidenceReviewWorklistHealthResult,
    ...
    return "\n".join(
        [
            ...
            _markdown_table(
                result.health_frame,
                ["worklist_id", "severity", "issue_code", "path_field", "issue_message", "suggested_action"],
            ),
            "",
        ]
    )
...
def _load_index(
    *,
    ...
    if index_path is not None:
        ...
        if not path.exists():
            issue = _issue(
                ...
                "Run pit-universe-evidence-review-worklist-index.",
            )
    ...
    frame = scan_pit_universe_evidence_review_worklist_artifacts(root)
    ...
    if not path.exists():
        issues.append(
            _issue(
                ...
                "Regenerate the PIT universe evidence review worklist artifact.",
            )
        )
        return None
    try:
    ...
    if missing:
        issues.append(
            _issue(
                ...
                "Regenerate the PIT universe evidence review worklist artifact with the current schema.",
            )
        )
    return frame
    ...
    if not path.exists():
        issues.append(
            _issue(
                ...
                "ERROR",
                "MISSING_REPORT",
                "PIT universe evidence review worklist report is missing.",
                "Regenerate the PIT universe evidence review worklist artifact.",
            )
        )


def _check_worklist_contract(
    row: dict[str, Any],
    metadata: dict[str, Any],
    worklist: pd.DataFrame,
    update_template: pd.DataFrame,
    issues: list[dict[str, Any]],
) -> None:
    _check_no_export_or_execution(row, metadata, issues)
    _check_safety_flags(row, metadata, worklist, issues)
    _check_non_approval_contract(row, metadata, worklist, update_template, issues)
...
def _check_no_export_or_execution(row: dict[str, Any], metadata: dict[str, Any], issues: list[dict[str, Any]]) -> None:
    ...
    for flag, code in false_flags.items():
        if _to_bool(metadata.get(flag)):
            issues.append(_metadata_issue(row, code, f"{flag}=true detected."))
    true_flags = {
        ...
        "worklist_only": "WORKLIST_ONLY_FLAG_MISSING",
    }
    for flag, code in true_flags.items():
        if not _to_bool(metadata.get(flag, False)):
            issues.append(_metadata_issue(row, code, f"{flag}=true is missing from metadata."))
...
def _check_safety_flags(
    ...
    for field, code in checks:
        if not _to_bool(metadata.get(field, False)):
            issues.append(_metadata_issue(row, code, f"{field}=true is missing from metadata."))
        if field in worklist.columns and (~worklist[field].map(_to_bool)).any():
            issues.append(_csv_issue(row, "worklist_csv_path", code, f"A row does not have {field}=true."))
    unsafe_flags = {
...
def _check_non_approval_contract(
    row: dict[str, Any],
    metadata: dict[str, Any],
    worklist: pd.DataFrame,
    update_template: pd.DataFrame,
    ...
    if _to_int(metadata.get("approved_count")) > 0 or _approved_count(worklist) > 0:
        issues.append(
            _csv_issue(
                row,
                "worklist_csv_path",
                "WORKLIST_APPROVED_ROWS",
                "Evidence review worklist artifact contains approved rows.",
            )
        )
    if _to_int(metadata.get("valid_for_signal_date_count")) > 0 or _true_count(worklist, "current_valid_for_signal_date") > 0:
        issues.append(
            _csv_issue(
                row,
                "worklist_csv_path",
                "WORKLIST_SET_VALID_FOR_SIGNAL_DATE",
                "Evidence review worklist artifact contains valid_for_signal_date=true rows.",
            )
        )
    if _to_int(metadata.get("authoritative_hint_count")) > 0 or _true_count(worklist, "hint_authoritative_for_pit") > 0:
        issues.append(
            _csv_issue(
                row,
                "worklist_csv_path",
                "AUTHORITATIVE_HINT_DETECTED",
                "Evidence review worklist hints must remain non-authoritative for PIT approval.",
            )
        )
    if "review_status" in update_template.columns:
        non_blank = update_template["review_status"].map(_string_or_empty).ne("")
        if non_blank.any():
            issues.append(
                _csv_issue(
                    row,
                    "update_template_path",
                    "UPDATE_TEMPLATE_PREAPPROVES_ROWS",
                    "Update template contains non-blank review_status values.",
                )
            )
    if "include_flag" in update_template.columns:
        included = update_template["include_flag"].map(_to_bool)
        if included.any():
            issues.append(
                _csv_issue(
                    row,
                    "update_template_path",
                    "UPDATE_TEMPLATE_AUTO_INCLUDE",
                    "Update template contains include_flag=true rows.",
                )
            )


def _metadata_issue(row: dict[str, Any], code: str, message: str) -> dict[str, Any]:
    return _issue(
        row,
        "metadata_path",
        ...
        code,
        message,
        "Regenerate evidence review worklist artifacts as report/template-only artifacts.",
    )


def _csv_issue(row: dict[str, Any], path_field: str, code: str, message: str) -> dict[str, Any]:
    return _issue(
        row,
        path_field,
        row.get(path_field),
        ...
        code,
        message,
        "Keep evidence review worklist outputs non-approval-only and rerun the worklist.",
    )
...
def _prepare_index_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in PIT_UNIVERSE_EVIDENCE_REVIEW_WORKLIST_INDEX_COLUMNS:
        if column not in output.columns:
            output[column] = ""
    return output[PIT_UNIVERSE_EVIDENCE_REVIEW_WORKLIST_INDEX_COLUMNS].reset_index(drop=True)
...
def _issue(
    ...
    return {
        ...
        "worklist_id": _string_or_empty(row.get("worklist_id")),
