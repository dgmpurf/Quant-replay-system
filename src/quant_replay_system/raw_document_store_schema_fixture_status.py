"""Status view for report-only raw document store schema fixture artifacts."""
...
class RawDocumentStoreSchemaFixtureStatusResult:
    latest_run_id: str
    status: str
    workflow_stage: str
    health_status: str
    ...
    production_raw_document_store_created: bool
    real_source_permission_created: bool
    real_data_fetched: bool
    raw_document_ingestion_created: bool
...
def run_raw_document_store_schema_fixture_status(
    *,
    root: str | Path = "outputs/reports/manual_diagnostics/raw_document_store_schema_fixture_v0_1",
    output_dir: str | Path = "outputs/reports/manual_diagnostics/raw_document_store_schema_fixture_v0_1/status",
) -> RawDocumentStoreSchemaFixtureStatusResult:
    sibling_root = Path(output_dir).parent
    index = build_raw_document_store_schema_fixture_index(root=root, output_dir=sibling_root / "index")
    health = check_raw_document_store_schema_fixture_health(root=root, output_dir=sibling_root / "health")
    if index.index_frame.empty:
        result = _no_artifact_result(output_dir, root)
    else:
        latest = index.index_frame.sort_values(["created_at", "raw_document_store_schema_fixture_id"]).iloc[-1].to_dict()
        if health.status == "FAIL":
            ...
            result = _result_from_latest(
                latest,
                status="PASS",
                stage=RAW_DOCUMENT_STORE_SCHEMA_FIXTURE_CREATED,
                health_status=health.status,
...
def _result_from_latest(
    ...
    *,
    ...
    summary = {
        ...
        "document_count": _to_int(latest.get("document_count")),
        ...
        "raw_document_store_schema_fixture_created": _to_bool(latest.get("raw_document_store_schema_fixture_created")),
    ...
    return RawDocumentStoreSchemaFixtureStatusResult(
        ...
        raw_document_store_schema_fixture_created=summary["raw_document_store_schema_fixture_created"],
        report_path=summary["report_path"],
        next_action=next_action,
        summary_frame=frame,
        artifact_paths=paths,
        warnings=[] if health_status == "PASS" else [f"Raw document store schema fixture health is {health_status}."],
        audit_metadata=_audit_metadata(root),
        **{flag: summary[flag] for flag in FORBIDDEN_METADATA_FALSE_FLAGS},
    )


def _no_artifact_result(output_dir: str | Path, root: str | Path) -> RawDocumentStoreSchemaFixtureStatusResult:
    summary = {
        "latest_run_id": "",
        "status": "PASS",
        "workflow_stage": NO_RAW_DOCUMENT_STORE_SCHEMA_FIXTURE,
        "health_status": "PASS",
        "document_count": 0,
        "validation_issue_count": 0,
        "report_only": True,
        "diagnostic_only": True,
        "raw_document_store_schema_fixture_created": False,
        **{flag: False for flag in FORBIDDEN_METADATA_FALSE_FLAGS},
        "report_path": "",
        "next_action": "Run raw-document-store-schema-fixture to create report-only raw document store schema fixture artifacts.",
    }
    paths = _paths(output_dir)
    frame = pd.DataFrame([summary], columns=SUMMARY_COLUMNS)
    return RawDocumentStoreSchemaFixtureStatusResult(
        ...
        summary_frame=frame,
        artifact_paths=paths,
        warnings=[f"No raw document store schema fixture artifacts found under {root}."],
        audit_metadata=_audit_metadata(root),
        **{flag: False for flag in FORBIDDEN_METADATA_FALSE_FLAGS},
    )


def _write(result: RawDocumentStoreSchemaFixtureStatusResult) -> None:
    ...
    paths["status_report"].write_text(
        "\n".join(
            [
                ...
                f"- raw_document_store_schema_fixture_created: {result.raw_document_store_schema_fixture_created}",
                "",
                "This is a report-only raw document store schema fixture status view.",
                "It does not create production raw document store state.",
                "It does not grant real source permission.",
                "It does not fetch real data or create raw document ingestion.",
                "It does not create factor observations, structured events, company exposure mappings, replay evidence bundles, buy-review eligibility, performance validation, broker behavior, API calls, orders, messages, advisory predictions, probabilities, or trading.",
                "",
...
def _paths(output_dir: str | Path) -> dict[str, Path]:
    ...
    return {
        ...
        "status_csv": artifact_dir / "raw_document_store_schema_fixture_status.csv",
        "status_report": artifact_dir / "raw_document_store_schema_fixture_status_report.md",
...
def _audit_metadata(root: str | Path) -> dict[str, Any]:
    return {
        ...
        "report_only": True,
        "diagnostic_only": True,
        "raw_document_store_schema_fixture_status_created": True,
        **{flag: False for flag in FORBIDDEN_METADATA_FALSE_FLAGS},
    }
