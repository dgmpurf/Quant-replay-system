"""Status view for official status evidence packet closure worklist artifacts."""
...
from quant_replay_system.historical_replay_official_status_evidence_packet_closure_worklist import SAFETY_FALSE_FIELDS
...
STATUS_COLUMNS = [
    ...
    "latest_missing_official_evidence_count",
    "latest_needs_manual_review_count",
    "latest_no_hit_review_needed_count",
    "latest_no_hit_accepted_context_count",
    "latest_packet_row_ready_not_pit_approved_count",
    "latest_profile_conflict_count",
    "latest_survivorship_warning_count",
    "latest_listed_status_missing_count",
    "latest_delisted_status_missing_count",
    "latest_st_status_missing_count",
    "latest_st_not_applicable_policy_missing_count",
    "latest_suspension_or_trading_status_missing_count",
    "latest_universe_membership_missing_count",
    "latest_source_id_missing_count",
    "latest_permission_class_missing_count",
    "latest_revision_id_missing_count",
    "latest_available_time_missing_count",
    "report_only",
    "diagnostic_only",
...
class HistoricalReplayOfficialStatusEvidencePacketClosureWorklistStatusResult:
    status: str
    latest_packet_worklist_run_id: str
    latest_status: str
...
def run_historical_replay_official_status_evidence_packet_closure_worklist_status(
    *,
    root: str | Path = DEFAULT_ROOT,
    output_dir: str | Path | None = None,
) -> HistoricalReplayOfficialStatusEvidencePacketClosureWorklistStatusResult:
    root_path = Path(root)
    out_dir = Path(output_dir) if output_dir is not None else root_path / "status"
    sibling_root = out_dir.parent
    index = build_historical_replay_official_status_evidence_packet_closure_worklist_index(
        root=root_path, output_dir=sibling_root / "index"
    )
    health = check_historical_replay_official_status_evidence_packet_closure_worklist_health(
        root=root_path, output_dir=sibling_root / "health"
    )

    if index.rows:
        latest = sorted(index.rows, key=lambda row: str(row.get("packet_worklist_run_id", "")))[-1]
        summary = _summary_from_latest(latest, health.status)
    else:
        summary = _no_artifact_summary(health.status)

    paths = _paths(out_dir)
    result = HistoricalReplayOfficialStatusEvidencePacketClosureWorklistStatusResult(
        status=STATUS_CREATED,
        latest_packet_worklist_run_id=str(summary.get("latest_packet_worklist_run_id", "")),
        ...
        warnings=[] if health.status.endswith("PASS_REPORT_ONLY") else [f"Official status worklist health is {health.status}."],
    )
    _write(result)
    return result
...
def _summary_from_latest(latest: dict[str, Any], health_status: str) -> dict[str, Any]:
    summary: dict[str, Any] = {
        "latest_packet_worklist_run_id": latest.get("packet_worklist_run_id", ""),
        ...
        "latest_missing_official_evidence_count": latest.get("missing_official_evidence_count", 0),
        ...
        "latest_no_hit_accepted_context_count": latest.get("no_hit_accepted_context_count", 0),
        "latest_packet_row_ready_not_pit_approved_count": latest.get("packet_row_ready_not_pit_approved_count", 0),
        ...
        "latest_listed_status_missing_count": latest.get("listed_status_missing_count", 0),
        "latest_delisted_status_missing_count": latest.get("delisted_status_missing_count", 0),
        "latest_st_status_missing_count": latest.get("st_status_missing_count", 0),
        "latest_st_not_applicable_policy_missing_count": latest.get("st_not_applicable_policy_missing_count", 0),
        "latest_suspension_or_trading_status_missing_count": latest.get(
            "suspension_or_trading_status_missing_count", 0
        ),
        "latest_universe_membership_missing_count": latest.get("universe_membership_missing_count", 0),
        "latest_source_id_missing_count": latest.get("source_id_missing_count", 0),
        "latest_permission_class_missing_count": latest.get("permission_class_missing_count", 0),
        "latest_revision_id_missing_count": latest.get("revision_id_missing_count", 0),
        "latest_available_time_missing_count": latest.get("available_time_missing_count", 0),
        "report_only": True,
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        ...
        "status_csv": output_dir / "historical_replay_official_status_evidence_packet_closure_worklist_status.csv",
        "status_md": output_dir / "historical_replay_official_status_evidence_packet_closure_worklist_status.md",
        "metadata_json": output_dir / "metadata.json",
    }


def _write(result: HistoricalReplayOfficialStatusEvidencePacketClosureWorklistStatusResult) -> None:
        ...
        json.dumps(
            {
                ...
                "latest_packet_worklist_run_id": result.latest_packet_worklist_run_id,
...
def _render_markdown(result: HistoricalReplayOfficialStatusEvidencePacketClosureWorklistStatusResult) -> str:
    return "\n".join(
        [
            "# Historical Replay Official Status Evidence Packet Closure Worklist Status",
            "",
            f"- Latest run id: `{result.latest_packet_worklist_run_id}`",
            ...
            "- Report-only status; no official evidence closure, PIT approval, replay, labels, training, model, buy-review, trading, or protected data-write behavior is authorized.",
            "",
        ]
    )
